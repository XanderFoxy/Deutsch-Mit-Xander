#!/usr/bin/env node
/* =====================================================================
   SONDE — FASSUNG 681: SIEGESLOHN
   ---------------------------------------------------------------------
   XANDER: „wenn ich gegen jemanden kämpfe und ich gewinne, dass da auch
   ne Punktzahl kommt … Erfahrungspunkte … man nimmt dem anderen auch
   nichts weg". Server (Rollback-Test): K.o. +10 Erfahrung, +2 fürs
   Ranking. Hier: die Meldung auf dem Telefon.
   ===================================================================== */
...
(async () => {
  ...
  await pg.evaluate(() => {
    window.__extra = { spiel_treffer: (a, ich) => ({ ok: true, zone: "kopf", schaden: 16, abgewehrt: 0, kaputt: true, ziel: { id: "11111111-1111-4111-8111-111111111111", name: "Bea", lp: 0, lp_max: 100, kaputt: true, mitspielen: true },
      ich: Object.assign({}, ich), gegenwehr: 0, lohn: 4, sieg: 2, sieg_xp: 10, waffe: a.p_waffe }) };
    const S = window.DMA_SPIEL.pruef.zustand(); S.waffe = "kartoffel"; window.__hinweise.length = 0; window.DMA_TONLOG.length = 0;
  });
  await pg.evaluate(() => document.querySelector('#lcPlaetze .lc-platz[data-lc-id="bea"]').scrollIntoView({ block: "nearest" }));
  await tick(500);
  const bea = await seite("bea");
  await pg.touchscreen.tap(bea.x, bea.y);
  await tick(2200);
  const m = await pg.evaluate(() => ({ meld: window.__hinweise.join(" | "), ruf: window.__rufe.some((r) => r.name === "spiel_treffer"), jubel: window.DMA_TONLOG.some((t) => t.name === "jubel") }));
  sage(m.ruf && /Bea ist K\.o\.! \+6 Punkte, \+10 Erfahrung – 2 davon zählen im Ranking/.test(m.meld), "K.o.: „Bea ist K.o.! +6 Punkte, +10 Erfahrung – 2 davon zählen im Ranking.“", m.meld);
  sage(m.jubel, "dazu Jubel", JSON.stringify(m));

  sage(konsolenFehler.length === 0, "keine Fehler in der Konsole", konsolenFehler.slice(0, 3).join(" | "));
  console.log("\n" + (fehler ? "Fassung 681: " + fehler + " rot." : "Fassung 681 auf dem Telefon: alles grün.") + "\n");
