...
(async () => {
  ...
  await tick(300);
  const rad = await pg.evaluate(() => ({ namen: [...document.querySelectorAll(".sp-rad-voll .sp-rad-sektoren text.sp-rad-klasse")].map((t) => t.textContent).join(","), n: document.querySelectorAll(".sp-rad-voll .sp-rad-waffe").length }));
  sage(/Meister$/.test(rad.namen) && rad.n === 16, "eigener Sektor „Meister“ im Rad (goldfarben), alle 16 Waffen erreichbar", JSON.stringify(rad));
