...
(async () => {
  ...
  const balken = await pg.evaluate(() => {
    ...
    return {
      ...
      /* Der Balken liegt UNTER dem Kreis, nicht über dem Gesicht. */
      unterDemKreis: besetzt.every((p) => {
        const b = p.querySelector(".sp-lp"), k = p.querySelector(".lc-kreis");
        return !b || b.getBoundingClientRect().top >= k.getBoundingClientRect().bottom - 1;
      }),
  ...
  sage(balken.unterDemKreis, "der Balken liegt unter dem Bild, die Gesichtsmitte bleibt frei");
