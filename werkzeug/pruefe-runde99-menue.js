...
const GRUPPEN = {
  ...
  /* Walkie-Talkie: „bei Putzen ist die Birne mit untergebracht, das
     macht keinen Sinn." */
  /* RUNDE 101 — Xander (Abnahme 10): „Die Spucke ist nicht mehr auffindbar
     unter Sauber machen." Putzen ist dort jetzt aufgeklappt: Schwamm,
     Lappen und Spucke stehen direkt da. */
  "Sauber machen": ["Schwamm und Eimer", "Putzlappen", "Draufspucken", "Wischer", "Waschmaschine"],
...
(async () => {
  ...
  const lehrer = async (chef) => pg.evaluate(async (chef) => {
    ...
    /* RUNDE 100 — XANDER (Walkie #87): „Es läßt mich immer den
       Livestream verlassen. Das soll so nicht sein." Der Knopf startet
       jetzt eine Runde IM Raum; das Klassenzimmer bleibt offen. */
    window.DMA_PRUEFUNG.platzMenue(ich); tipp("Lehrer"); tipp("Stadt \u00b7 Land \u00b7 Fluss starten");
    await new Promise((f) => setTimeout(f, 400));
    const slfLaeuft = window.LiveChat.slfLaeuft();
    const slfBlatt = document.querySelectorAll("#lcSlfLive input").length;
    const raumOffen = (document.getElementById("sub-stadtlandfluss") || { dataset: {} }).dataset.active !== "true";
    window.DMA_PRUEFUNG.platzMenue(ich); tipp("Lehrer");
    const panel4 = worte();
    tipp("Stadt \u00b7 Land \u00b7 Fluss beenden");
    await new Promise((f) => setTimeout(f, 300));
    return { lehrer: true, panel, schiffe, panel2, panel3, aufgabeNoch, slfLaeuft, slfBlatt, raumOffen, panel4,
      slfAus: !window.LiveChat.slfLaeuft() && !document.getElementById("lcSlfLive") };
  }, chef);
  ...
  sage(lp.panel && lp.panel.indexOf("Schiffe versenken starten") >= 0 && lp.panel.indexOf("Aufgabe stellen") >= 0
    && lp.panel.indexOf("Stadt \u00b7 Land \u00b7 Fluss starten") >= 0, "... darin Schiffe versenken, Aufgaben, Stadt · Land · Fluss",
  ...
  sage(lp.slfLaeuft && lp.slfBlatt >= 3 && lp.raumOffen,
    "Stadt · Land · Fluss startet IM Klassenzimmer — Blatt unter der Bühne, niemand verlässt den Raum",
    lp.slfBlatt + " Felder");
  sage(lp.panel4 && lp.panel4.indexOf("Stadt \u00b7 Land \u00b7 Fluss beenden") >= 0 && lp.slfAus,
    "... danach heisst der Knopf „beenden“ und beendet die Runde");
  ...
  console.log("\n10  KRAN-MENUE: PLAETZE WIE AUF DER BUEHNE, NICHTS RAGT UEBER DEN RAND\n");
  /* XANDER (Walkie #85): „Im Menü möchte ich die Auswahl Plätze logisch
     untereinander so wie in der Bühnen Ansicht sortiert haben auch beim
     Android ohne Verschiebung." Gemessen vorher (390 px): Menue bei
     72–402 px, also 12 px ueber dem Rand; Plaetze in Sechserreihen. */
  for (const [breite, hoehe, wer] of [[390, 844, "Cem"], [412, 915, "Emil"]]) {
    const pk = await br.newPage({ viewport: { width: breite, height: hoehe }, isMobile: true, hasTouch: true });
    await pk.addInitScript(() => { try { localStorage.setItem("dma_tour_seen", "1"); } catch (e) {} });
    await pk.goto("http://127.0.0.1:" + srv.address().port + "/index.html", { waitUntil: "domcontentloaded" });
    await pk.waitForFunction(() => window.LiveChat && window.LiveChat.pruefSitz && window.DMA_PRUEF, { timeout: 25000 });
    const km = await pk.evaluate(async (wer) => {
      window.LiveChat.pruefSitz({ lage: "drin", ichId: "ich", ichName: "Alex", seit: 1000, zuruecksetzen: true,
      ...
      const plaetze = [...document.querySelectorAll("#lcPlaetze .lc-platz")];
      const platz = plaetze.find((p) => p.textContent.indexOf(wer) >= 0);
      ...
      finde(/Kran/).click();
      await new Promise((f) => setTimeout(f, 400));
      const m = document.getElementById("lcPlatzMenue"), r = m.getBoundingClientRect();
      const zellen = [...m.querySelectorAll(".lc-hebenwahl-platz")];
      const oben0 = plaetze[0].offsetTop;
      const buehneSpalten = plaetze.filter((p) => Math.abs(p.offsetTop - oben0) < 4).length;
      const z0 = zellen.length ? zellen[0].getBoundingClientRect().top : 0;
      const menueSpalten = zellen.filter((z) => Math.abs(z.getBoundingClientRect().top - z0) < 4).length;
      const nummern = zellen.map((z) => (z.querySelector(".lc-hebenwahl-nr") || {}).textContent).join(",");
      const buehneNr = plaetze.map((p) => p.dataset.lcPlatz).join(",");
      return { links: Math.round(r.left), rechts: Math.round(r.right), breite: window.innerWidth,
               menueSpalten, buehneSpalten, nummern, buehneNr,
               angeln: Boolean([...m.querySelectorAll("button")].find((b) => /Angeln/.test(b.textContent))) };
    }, wer);
    await pk.close();
    if (km.fehlt) { sage(false, breite + " px: Kran-Menue geht auf"); continue; }
    sage(km.links >= 8 && km.rechts <= km.breite - 8, breite + " px: das Menue bleibt ganz auf dem Bildschirm",
      km.links + "–" + km.rechts + " von " + km.breite);
    sage(km.menueSpalten === km.buehneSpalten && km.nummern === km.buehneNr,
      breite + " px: Plaetze in derselben Anordnung wie auf der Buehne",
      km.menueSpalten + " Spalten (Buehne " + km.buehneSpalten + "), " + km.nummern);
    sage(!km.angeln, breite + " px: im Kran-Menue steht kein „Angeln“");
  }

  console.log("\n11  STADT · LAND · FLUSS MIT ZWEI GERAETEN\n");
  /* Alex startet, Bea ruft „Stopp!", ihr Blatt geht als persoenliche
     Post an Alex, Alex wertet aus, beide sehen dasselbe Ergebnis.
     Der Bote spielt die Leitung: was A rundsendet, bekommt B, und
     umgekehrt. */
  {
    const LEUTE = { ich: { id: "ich", name: "Alex", seit: 1000 }, p0: { id: "p0", name: "Bea", seit: 2000 } };
    const geraet = async (ichId) => {
      const g = await br.newPage({ viewport: { width: 390, height: 844 } });
      await g.addInitScript(() => { try { localStorage.setItem("dma_tour_seen", "1"); } catch (e) {} });
      await g.goto("http://127.0.0.1:" + srv.address().port + "/index.html", { waitUntil: "domcontentloaded" });
      await g.waitForFunction(() => window.LiveChat && window.LiveChat.pruefSitz && window.DMA_PRUEF, { timeout: 25000 });
      await g.evaluate(async ({ ichId, LEUTE }) => {
        const leute = {}; Object.keys(LEUTE).forEach((k) => { if (k !== ichId) leute[k] = LEUTE[k]; });
        window.LiveChat.pruefSitz({ lage: "drin", ichId, ichName: LEUTE[ichId].name, seit: LEUTE[ichId].seit, zuruecksetzen: true, leute });
        document.querySelectorAll(".view,.subview").forEach((v) => { v.dataset.active = "false"; });
        let e = document.getElementById("livechatArea");
        while (e && e !== document.body) { if (e.dataset && "active" in e.dataset) e.dataset.active = "true"; e = e.parentElement; }
        window.DMA_PRUEF.neuZeichnen();
        await new Promise((f) => setTimeout(f, 400));
        window.__raus = []; window.__post = [];
        window.LiveChat.pruefPost((pk) => window.__raus.push(pk));
        window.LiveChat.pruefPostAbfangen((an, pk) => window.__post.push(pk));
      }, { ichId, LEUTE });
      return g;
    };
    const A = await geraet("ich"), B = await geraet("p0");
    const bote = async () => {
      for (const [von, zu, wer] of [[A, B, "ich"], [B, A, "p0"]]) {
        const w = await von.evaluate(() => { const r = { raus: window.__raus, post: window.__post };
          window.__raus = []; window.__post = []; return r; });
        for (const pk of w.raus) await zu.evaluate((pk) => window.LiveChat.pruefEmpfangen(pk), pk);
        for (const pk of w.post.filter((x) => x.art === "slfpost")) {
          await zu.evaluate(({ pk, wer }) => window.LiveChat.pruefPostEmpfangen(Object.assign({}, pk, { von: wer, vonName: wer === "p0" ? "Bea" : "Alex" })), { pk, wer });
        }
      }
    };
    await A.evaluate(() => window.LiveChat.pruefBefehl("/slf"));
    await bote();
    const blatt = async (g) => g.evaluate(() => ({ b: (document.querySelector(".lc-slf-buchstabe") || {}).textContent || "",
      felder: document.querySelectorAll("#lcSlfLive input").length }));
    const a1 = await blatt(A), b1 = await blatt(B);
    sage(a1.felder >= 3 && b1.felder === a1.felder && a1.b && a1.b === b1.b,
      "beide sehen das Blatt mit demselben Buchstaben", "Alex " + a1.b + "/" + a1.felder + ", Bea " + b1.b + "/" + b1.felder);
    const tippe = (g, w) => g.evaluate((w) => document.querySelectorAll("#lcSlfLive input").forEach((inp, i) => {
      if (!w[i]) return; inp.value = w[i]; inp.dispatchEvent(new Event("input")); }), w);
    const B0 = a1.b;
    await tippe(A, [B0 + "stadtA", B0 + "land", B0 + "flussA"]);
    await tippe(B, [B0 + "stadtB", B0 + "land", ""]);
    await B.evaluate(() => window.LiveChat.slfStopp());
    await bote();
    const eilig = await A.evaluate(() => (document.getElementById("lcSlfHinweis") || {}).textContent || "");
    sage(/Bea hat Stopp gerufen/.test(eilig), "Beas „Stopp!“ kommt bei Alex an — noch 3 Sekunden", eilig);
    await A.waitForTimeout(3600);
    await bote();
    await A.waitForTimeout(300);
    const erg = async (g) => g.evaluate(() => ({ phase: (window.LiveChat.slfStand() || {}).phase,
      summe: [...document.querySelectorAll(".lc-slf-summe td")].map((t) => Number(t.textContent)),
      namen: [...document.querySelectorAll(".lc-slf-tabelle thead th")].map((t) => t.textContent).filter(Boolean) }));
    const ea = await erg(A), eb = await erg(B);
    /* Stadt: je eigenes Wort 10; Land: gleiches Wort je 5; Fluss: nur Alex 20. */
    const soll = { Alex: 35, Bea: 15 };
    const ist = {}; ea.namen.forEach((n, i) => { ist[n] = ea.summe[i]; });
    sage(ea.phase === "ergebnis" && eb.phase === "ergebnis" && ist.Alex === soll.Alex && ist.Bea === soll.Bea
      && JSON.stringify(ea.summe) === JSON.stringify(eb.summe),
      "Auswertung bei beiden gleich: Alex 35, Bea 15 (20 nur du, 10 eigenes, 5 gleiches Wort)", JSON.stringify(ist));
