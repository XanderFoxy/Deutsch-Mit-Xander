...
(async () => {
  ...
  await pg.evaluate(() => { try { localStorage.removeItem("dma_spiel_leiste"); } catch (e) {} const S = window.DMA_SPIEL.pruef.zustand();
  ...
  await tippe('.sp-leiste-ed [data-k="faehig"]');
  await tippe('.sp-schnellmenue [data-s="leisteschieb"][data-r="-1"]');
  await tippe('.sp-schnellmenue [data-s="leisteschieb"][data-r="-1"]');
  await tick(200);
  r = await pg.evaluate(() => ({ menue: [...document.querySelectorAll(".sp-leiste-ed .sp-tasche")].map((e) => e.dataset.k), gemerkt: JSON.parse(localStorage.getItem("dma_spiel_leiste") || "[]") }));
  sage(r.menue.indexOf("faehig") === 2 && r.menue.indexOf("faehig") < r.menue.indexOf("trank") && r.menue.indexOf("faehig") < r.menue.indexOf("super"), "Tier-Fähigkeit zweimal nach links: jetzt vor Tränken und Superkraft", JSON.stringify(r.menue));
  ...
  sage(r.indexOf("faehig") < r.indexOf("trank"), "nach dem Neuladen gilt sie weiter", JSON.stringify(r));
