#!/usr/bin/env node
/* =====================================================================
   SONDE — FASSUNG 675: EIN PUNKTESYSTEM FÜR SPIEL UND SEITE
   ---------------------------------------------------------------------
   XANDER: „Das, was die Leute in dem Livestream Spiel machen muss Ihnen
   alles belohnt werden in ihrem ganz normalen Rankings … die Sachen mit den
   Tower Defense Spiel oder Strategiespiel. Das muss auch für sich in unseren
   Punkten bisschen bemerkbar machen".
   Die Gutschrift selbst macht die Datenbank (Trigger, im Rollback geprüft);
   hier: am Ende von Tower Defense und Rundenkampf wird der Lohn geholt,
   genau einmal, mit dem Spielbeginn als Zeitpunkt.
   ===================================================================== */
...
(async () => {
  ...
  await pg.evaluate(() => {
    ...
    const klient = { rpc: (name, args) => {
      ...
      else if (name === "spiel_extra_lohn") { window.__lohnArgs = args; data = { ok: true, lohn: 14, richtig: 5, heute: 14 }; }
  ...
  console.log("\nLOHN AM ENDE DER EXTRA-SPIELE\n");
  const td = await pg.evaluate(async () => {
    const P = window.DMA_SPIEL.pruef; P.extraOeffnen("td"); await new Promise((r) => setTimeout(r, 300));
    const T = P.td(); T.welle = 4; const start = T.start;
    window.__rufe.length = 0;
    /* Die Teufel sind durch: Spielende. */
    T.leben = 0; T.feinde = [{ x: 0, y: 0, weg: 999, lp: 1 }];
    await new Promise((r) => setTimeout(r, 1500));
    const rufe = window.__rufe.filter((r) => r.name === "spiel_extra_lohn");
    return { start: Boolean(start), rufe: rufe.length, args: rufe[0] && rufe[0].args, meld: window.__hinweise.slice(-2).join(" | "), ende: T.ende };
  });
  sage(td.start && td.ende && td.rufe === 1 && td.args && td.args.p_art === "td" && /T/.test(td.args.p_seit), "Tower Defense zu Ende: genau ein Lohn-Ruf mit Spielbeginn", JSON.stringify(td));
  sage(/\+14 Punkte/.test(td.meld) && /Ranking/.test(td.meld), "die Meldung nennt den Lohn und dass er im Ranking zählt", td.meld);
  await pg.evaluate(() => window.DMA_SPIEL.pruef.extraSchliessen());
  const st = await pg.evaluate(async () => {
    const P = window.DMA_SPIEL.pruef; P.extraOeffnen("strat"); await new Promise((r) => setTimeout(r, 300));
    /* Eine Partie gegen Bea beginnen und Bea aufgeben lassen. */
    window.__rufe.length = 0;
    P.stratEmpfangen({ typ: "einladung", von: "bea", name: "Bea", partie: "p675" });
    const ja = document.querySelector('.sp-frage [data-ja="1"]'); if (ja) ja.click();
    await new Promise((r) => setTimeout(r, 300));
    const S = P.st(); const start = S.start;
    P.stratEmpfangen({ typ: "ende", partie: S.id, von: "bea" });
    await new Promise((r) => setTimeout(r, 400));
    const rufe = window.__rufe.filter((r) => r.name === "spiel_extra_lohn");
    return { start: Boolean(start), rufe: rufe.length, args: rufe[0] && rufe[0].args };
  });
  sage(st.start && st.rufe === 1 && st.args && st.args.p_art === "strat" && st.args.p_ergebnis === 10, "Rundenkampf: gibt der Gegner auf, kommt genau ein Lohn-Ruf (als Sieg)", JSON.stringify(st));
  const regel = await pg.evaluate(() => { window.DMA_SPIEL.menue("deutsch"); return document.querySelector("#spPanel").textContent; });
  sage(/zählt auch im Ranking der Seite/.test(regel), "im Deutsch-Reiter steht: alles zählt auch im Ranking der Seite");
  sage(konsolenFehler.length === 0, "keine Fehler in der Konsole", konsolenFehler.slice(0, 3).join(" | "));
  console.log("\n" + (fehler ? "Fassung 675: " + fehler + " rot." : "Fassung 675 auf dem Telefon: alles grün.") + "\n");
