...
(async () => {
  ...
  const probe = (chat, klasse, von, warte, blick) => pg.evaluate(([chat, klasse, von, warte, blick]) => new Promise((ok) => {
    ...
    pl.dataset.tierZeit = 0; window.DMA_TONLOG.length = 0; window.__tierReaktionen.length = 0; window.__tierLaute = [];
    const gesehen = {};
    const merk = () => {
    ...
    /* Fassung 695: was ein Tier „sagt", ist jetzt ein Laut mit gezeichnetem
       Zeichen — die Worte stehen in __tierLaute (keine Blase mehr). */
    setTimeout(() => { clearInterval(uhr); (window.__tierLaute || []).forEach((l) => { gesehen["sagt:" + l.text] = 1; gesehen["laut:" + l.laut] = 1; }); ok({ art: (window.__tierReaktionen[0] || {}).art, gesehen: Object.keys(gesehen), ton: window.DMA_TONLOG.map((t) => t.name) }); }, warte);
  }), [chat, klasse, von, warte, blick]);
  /* Fassung 695: die gesprochenen Laute kommen aus der eigenen Stimme des Tiers
     (ton/tier-<art>-<laut>) statt aus dem gerechneten Ton. */
  const STIMME = { "stimme:mmh": /^tier-.+-genuss$/, "stimme:frage": /^tier-.+-frage$/, "stimme:wow": /^tier-.+-ruf$/, "stimme:kichern": /^tier-.+-ruf$/, "stimme:ooh": /^tier-.+-(frage|ruf)$/ };
  const hat = (r, ...w) => w.every((x) => r.gesehen.includes(x) || r.ton.includes(x) || (STIMME[x] && r.ton.some((t) => STIMME[x].test(t))));
  ...
  /* Fassung 695: erst schauen sie hoch und schnuppern, bis die Sahne fertig ist;
     ohne echte Haube (hier nur der Effekt-Aufruf) lecken sie dann am Platz. */
  r = await probe("ich", "lc-sahne", "Bea", 5600);
  sage(r.art === "schlecken" && hat(r, "sp-tier-hochgucken", "sp-tier-leckt", "zunge", "lecken", "mampf", "ruelps", "herz") && sagt(r, /Rülps/), "Sahne: hochschauen, lecken, schmatzen, rülpsen", JSON.stringify(r));
  ...
  const jagd = await pg.evaluate(() => new Promise((ok) => { window.DMA_TONLOG.length = 0; window.__tierLaute = [];
    ...
    setTimeout(() => ok({ bewegt: [...pl.querySelectorAll(".sp-tier, .sp-flugtier")].some((t) => t.getAnimations().length > 0), sagt: (window.__tierLaute || []).map((l) => l.text), /* Fassung 695: Laute statt Blasen */ ton: window.DMA_TONLOG.map((t) => t.name) }), 700); }));
  ...
  const reise = await pg.evaluate(() => new Promise((ok) => { window.__tierLaute = [];
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { erg.tausch = (window.__tierLaute || []).map((l) => l.text).filter((t) => /Hä|\?!/.test(t)); /* Fassung 695 */ }, 900);
