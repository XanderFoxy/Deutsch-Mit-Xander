#!/usr/bin/env node
/* =====================================================================
   SONDE — FASSUNG 663: KLASSEN UND DEUTSCHLAND-WISSEN
   ---------------------------------------------------------------------
   XANDER: „kann man in anderen Klassen spielen? … wie wir das in unserem
   Hauptsystem haben auf der Webseite, dass man als Deutsch Profi spielt"
   und „Deutschland-Wissen". Android-Telefon (360 px), echte Fingertipps.
   ===================================================================== */
...
(async () => {
  ...
  console.log("\nKLASSE WÄHLEN\n");
  await pg.evaluate(() => { window.__ich.level = 3; window.__ich.klasse = null; });
  await zurMehr("klasse");
  const k0 = await pg.evaluate(() => ({ text: (document.querySelector(".sp-schnellmenue .sp-sm-klein") || {}).textContent, zu: [...document.querySelectorAll('.sp-schnellmenue [data-s="klassewahl"]')].every((b) => b.disabled), n: document.querySelectorAll(".sp-schnellmenue .sp-klasse").length }));
  sage(k0.n === 5 && k0.zu && /ab Level 5/.test(k0.text), "fünf Klassen, unter Level 5 noch gesperrt", JSON.stringify(k0));
  await pg.evaluate(() => { window.__ich.level = 6; });
  await zurMehr("klasse");
  const namen = await pg.evaluate(() => [...document.querySelectorAll(".sp-schnellmenue .sp-klasse b")].map((b) => b.textContent).join(", "));
  sage(namen === "Grammatik-Profi, Sprachkünstler, Logiker, Wissenschaftler, Abenteurer", "wie auf der Webseite: Grammatik-Profi, Sprachkünstler, Logiker, Wissenschaftler, Abenteurer", namen);
  await pg.locator(".sp-schnellmenue").screenshot({ path: "/tmp/claude-0/p-663-klassen.png" }).catch(() => {});
  await pg.evaluate(() => document.querySelector('.sp-schnellmenue [data-s="klassewahl"][data-k="wissenschaftler"]').click());
  await tick(300);
  /* Die Meldung steht ein paar Sekunden oben in der Kopfzeile – danach wieder Level und Klasse. */
  const meld = await pg.evaluate(() => window.__hinweise.slice(-1)[0]);
  await pg.evaluate(() => { const S = window.DMA_SPIEL.pruef.zustand(); S.menueMeldung = null; window.DMA_SPIEL.pruef.schnellZeichnen(true); });
  await tick(150);
  const k1 = await pg.evaluate((meld) => ({ ruf: (window.__rufe.filter((r) => r.name === "spiel_klasse").pop() || {}).args, m: meld, ist: (document.querySelector(".sp-schnellmenue .sp-troph-hat b") || {}).textContent,
    kopf: (document.querySelector(".sp-schnellmenue .sp-sm-kopf") || {}).textContent, raus: window.__raus.filter((p) => p.ereignis === "stand").pop() }), meld);
  sage(k1.ruf && k1.ruf.p_klasse === "wissenschaftler" && /Du bist jetzt Wissenschaftler/.test(k1.m), "Wählen: „Du bist jetzt Wissenschaftler“ mit dem Vorteil", k1.m);
  sage(k1.ist === "Wissenschaftler" && /Lv 6 Wissenschaftler/.test(k1.kopf), "die Klasse ist markiert und steht oben neben dem Level", k1.kopf);
  sage(Boolean(k1.raus && k1.raus.stand && k1.raus.stand.klasse === "wissenschaftler"), "die anderen im Raum sehen die Klasse (am Ring)");

  console.log("\nDEUTSCHLAND-WISSEN\n");
  await zurMehr("deutsch");
  const dk = await pg.evaluate(() => !!document.querySelector('.sp-schnellmenue .sp-sm-arten [data-k="deutschland"]'));
  sage(dk, "im Deutsch-Reiter gibt es „Deutschland“");
  await pg.evaluate(() => document.querySelector('.sp-schnellmenue .sp-sm-arten [data-k="deutschland"]').click());
  await tick(300);
  const auf = await pg.evaluate(() => (window.__rufe.filter((r) => r.name === "spiel_aufgabe").pop() || {}).args);
  sage(auf && auf.p_kategorie === "deutschland", "ein Tipp holt eine Deutschland-Frage", JSON.stringify(auf));

  sage(konsolenFehler.length === 0, "keine Fehler in der Konsole", konsolenFehler.slice(0, 3).join(" | "));
  console.log("\n" + (fehler ? "Fassung 663: " + fehler + " rot." : "Fassung 663 auf dem Telefon: alles grün.") + "\n");
