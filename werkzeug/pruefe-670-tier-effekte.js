#!/usr/bin/env node
/* =====================================================================
   SONDE — FASSUNG 670: JEDES TIER MIT EIGENEM EFFEKT, DAS EINHORN GLÄNZT
   ---------------------------------------------------------------------
   Funk 132, XANDER: „Verbessere bitte mal das Einhorn … es ist sein
   Deluxe Tier und es hat nur so einen schwachen Effekt … die Tiere die man
   kauft die sollen alle einen individuellen … starken Effekt".
   ===================================================================== */
...
(async () => {
  ...
  console.log("\nDAS EINHORN\n");
  const einhorn = await pg.evaluate(async () => {
    const zaehl = {}; const mo = new MutationObserver((l) => l.forEach((m) => m.addedNodes.forEach((n) => { if (n.nodeType === 1) { const k = String(n.className).split(" ")[0]; zaehl[k] = (zaehl[k] || 0) + 1; } })));
    mo.observe(document.body, { childList: true });
    window.DMA_SPIEL.pruef.tierAngriff("ich", "bea", "einhorn");
    await new Promise((r) => setTimeout(r, 900));
    const glanz = !!document.querySelector(".sp-tier-sprung.sp-tier-einhorn.sp-tier-glanz");
  ...
  await pg.screenshot({ path: "/tmp/claude-0/p-670-einhorn.png", clip: box });
  await tick(2600);
  ...
  for (const art of Object.keys(merkmal)) {
    erg[art] = await pg.evaluate(async ([art, kl]) => {
      let n = 0; const mo = new MutationObserver((l) => l.forEach((m) => m.addedNodes.forEach((x) => { if (x.nodeType === 1 && x.classList.contains(kl)) n++; })));
      mo.observe(document.body, { childList: true });
      window.DMA_SPIEL.pruef.tierAngriff("bea", "ich", art);
  ...
  sage(Object.values(erg).every((n) => n >= 1), "Stachelmonster schießt Stacheln, Schäferhund bellt Wellen, Dackel knurrt, Füchse funkeln, Eule/Greif verlieren Federn, Storch klappert", JSON.stringify(erg));

  sage(konsolenFehler.length === 0, "keine Fehler in der Konsole", konsolenFehler.slice(0, 3).join(" | "));
  console.log("\n" + (fehler ? "Fassung 670: " + fehler + " rot." : "Fassung 670 auf dem Telefon: alles grün.") + "\n");
