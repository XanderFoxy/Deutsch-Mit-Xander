#!/usr/bin/env node
/* =====================================================================
   SONDE — FASSUNG 710: AUFTRITT – TON UND BILD ZUSAMMEN, GESICHT IM KREIS (Funk 156)
   ---------------------------------------------------------------------
   XANDER (Funk 156): „Die Profil Intro Animation funktioniert noch nicht
   so ganz sie hängt vom Sound oft hinterher … das Profilbild ist immer
   noch nach unten gerutscht man erkennt mein Gesicht gar nicht".
   Geprüft: der erste Ton kommt nie vor dem ersten Bild der Animation –
   auch wenn das Foto langsam entpackt wird oder die Seite gerade beschäftigt
   ist; der runde Ausschnitt zeigt bei einem Hochformat-Foto das Gesicht.
   ===================================================================== */
...
(async () => {
  ...
  const bildUrl = await pg.evaluate(() => { const c = document.createElement("canvas"); c.width = 300; c.height = 450; const g = c.getContext("2d"); g.fillStyle = "#88a"; g.fillRect(0, 0, 300, 450);
    g.fillStyle = "#f2c9a0"; g.beginPath(); g.arc(150, 125, 75, 0, 7); g.fill(); g.fillStyle = "#c33"; g.fillRect(0, 260, 300, 190); return c.toDataURL("image/png"); });
  await pg.evaluate((u) => { const k = document.querySelector('#lcPlaetze .lc-platz[data-lc-id="bea"] .lc-kreis'); const i = k.querySelector("img.lc-avatar"); i.src = u; i.style.display = "block"; }, bildUrl);
  await tick(500);
  const lauf = (vorher) => pg.evaluate(async (vorher) => {
    ...
    const an = await warte(); const start = an.startTime - t0;
    await new Promise((o) => setTimeout(o, 3000));
    if (window.__altDecode) { HTMLImageElement.prototype.decode = window.__altDecode; window.__altDecode = null; }
    const t = window.DMA_TONLOG.map((x) => ({ n: x.name, t: Math.round(x.wann - t0) }));
    return { animStart: Math.round(start), toene: t };
  }, vorher);
  const cdp = await ctx.newCDPSession(pg);
  for (const fall of ["normal", "langsam", "beschaeftigt"]) {
    await cdp.send("Emulation.setCPUThrottlingRate", { rate: fall === "normal" ? 1 : 4 });
    const r = await lauf(fall);
    const erster = r.toene[0], bremse = r.toene.find((x) => x.n === "bremse");
    sage(erster && erster.t >= r.animStart - 20 && erster.t <= r.animStart + 120, fall + ": der erste Ton kommt mit dem ersten Bild der Animation (nicht davor)", JSON.stringify({ animStart: r.animStart, erster }));
    sage(bremse && Math.abs(bremse.t - r.animStart - 0.34 * 3600) < 150, fall + ": die Bremse quietscht, wenn der Wagen hält (34 % der Fahrt nach dem Start)", JSON.stringify({ bremse, soll: Math.round(r.animStart + 0.34 * 3600) }));
    await tick(1200);
  }
  await cdp.send("Emulation.setCPUThrottlingRate", { rate: 1 });
  console.log("\nDAS GESICHT IM KREIS\n");
  const r = await pg.evaluate(() => { const i = document.querySelector('#lcPlaetze .lc-platz[data-lc-id="bea"] img.lc-avatar'); return getComputedStyle(i).objectPosition; });
  sage(r === "50% 22%", "Hochformat-Foto: der runde Ausschnitt liegt oben, dort wo das Gesicht ist (vorher Mitte = Hals)", r);
  const pl = await pg.$('#lcPlaetze .lc-platz[data-lc-id="bea"] .lc-kreis');
  await pl.scrollIntoViewIfNeeded(); await tick(300);
  if (process.env.BILD) await pl.screenshot({ path: process.env.BILD + "-kreis.png" });
  const farbe = await pg.evaluate(async () => { const i = document.querySelector('#lcPlaetze .lc-platz[data-lc-id="bea"] img.lc-avatar'), r = i.getBoundingClientRect();
    /* Welcher Teil des Fotos liegt in der Kreismitte? object-position 50% 22%: die Mitte des Kreises zeigt im Foto die Höhe … */
    const iw = i.naturalWidth, ih = i.naturalHeight, s = Math.max(r.width / iw, r.height / ih), sichtH = r.height / s, oben = (ih - sichtH) * 0.22, mitteY = oben + sichtH / 2;
    return { mitteImFoto: Math.round(mitteY), gesicht: mitteY > 50 && mitteY < 200 }; });
  sage(farbe.gesicht, "die Kreismitte zeigt das Gesicht (im Testbild zwischen 50 und 200 px), nicht mehr den roten Pulli", JSON.stringify(farbe));
  sage(konsolenFehler.length === 0, "keine Seitenfehler", konsolenFehler.join(" | "));
  console.log("\nFassung 710 (Auftritt): " + (fehler ? fehler + " rot." : "alles grün."));
