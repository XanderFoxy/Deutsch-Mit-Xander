...
(async () => {
  ...
  /* WALKIE #223 — „Noch nicht synchron": die Zehntel kommen jetzt aus
     derselben Uhr wie der Blitz (bombenUhr), 20 Stueck ab 100 ms, und
     bei 2100 ms steht in DEMSELBEN Bild das 💥 und knallt es. */
  sage(/20 - Math\.floor\(Math\.max\(0, t - 100\) \/ 100\)/.test(app)
    && /if \(!geplatzt && t >= 2100\)/.test(app) && /\}, 4600, null\);/.test(app),
