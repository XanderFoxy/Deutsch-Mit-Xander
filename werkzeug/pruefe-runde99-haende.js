...
(async () => {
  ...
  for (const h of HAENDE) {
    ...
    masse[h.art] = await pg.evaluate(() => {
      /* RUNDE 101 — die Hand greift jetzt SEITLICH (78 Grad gedreht). Ein
         Umrechteck misst dann quer statt laengs. Fuer die Masse wird die
         Drehung der ganzen Hand deshalb abgenommen; die Gelenke bleiben,
         wie sie sind. */
      const vorn = document.querySelector(".lc-riesenhand-vorn");
      vorn.getAnimations().forEach((an) => { try { an.cancel(); } catch (e) {} });
      vorn.style.transform = "translate(-50%, -50%)";
  ...
  for (let k = 0; k <= 40; k++) {
    ...
    const p = await pgS.evaluate((tt) => {
      ...
      const last = document.querySelector(".lc-riesenhand-last");
      let lastWinkel = null;
      if (last) { const ml = new DOMMatrixReadOnly(getComputedStyle(last).transform); lastWinkel = Math.round(Math.atan2(ml.b, ml.a) * 1800 / Math.PI) / 10; }
      return {
        lastWinkel: lastWinkel,
  ...
  if (bahn.length < 30) {
    ...
    /* RUNDE 101 — XANDER (Abnahme 6): „Greifen nicht seitlich". Jetzt
       gilt: sie kommt waagerecht (mindestens 60 Grad) und BLEIBT beim
       Tragen seitlich; gerade bleiben muss das BILD, nicht die Hand. */
    ...
    sage(maxWinkel >= 60, "Sie kommt waagerecht von der Seite (nicht von oben)",
    ...
    sage(mittelWinkel >= 60, "Beim Tragen haelt sie das Bild weiter von der Seite",
      mittelWinkel.toFixed(1) + " Grad");
    const bildMitte = bahn.filter((p) => p.t >= 0.42 && p.t <= 0.60 && p.lastWinkel !== null);
    const bildSchief = Math.max.apply(null, bildMitte.map((p) => Math.abs(p.lastWinkel)));
    sage(bildMitte.length && bildSchief <= 10, "und das Bild selbst haengt dabei nicht schief",
      bildSchief.toFixed(1) + " Grad");
  }
