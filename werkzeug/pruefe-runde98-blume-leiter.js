...
(async () => {
  ...
  const blume = await pg.evaluate(async () => {
    ...
    /* „immer schön im Zentrum arbeitest mit dem Blumenkopf":
       Schwerpunkt der schon offenen Blaetter, Abstand zur Bildmitte. */
    const schwer = () => {
      const m = mitte();
      const auf = [...document.querySelectorAll(".lc-blbl")]
        .filter((b) => b.getBoundingClientRect().width > 4
          && Number(getComputedStyle(b).opacity) > 0.5);
      ...
      return Math.round(Math.hypot(x / auf.length - m.x, y / auf.length - m.y));
    };
    const spur = [];
    let versatz = 0;
    let stielHoch = 0;
    for (let i = 0; i < 24; i++) {
      ...
      spur.push(offen());
      versatz = Math.max(versatz, schwer());
