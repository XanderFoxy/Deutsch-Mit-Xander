...
window.LiveChat = (function () {
  ...
  function schiffeStarten(roh) {
    ...
    /* DIE AUSLOSUNG. Fisher-Yates ueber alle sechzehn Felder, dann
       bekommt der Erste der Reihe das erste gezogene Feld, der
       Zweite das zweite und so fort. Zwei koennen damit gar nicht
       im selben Feld landen — und keiner erfaehrt etwas ueber ein
       fremdes Feld, weil niemand mehr eines anklickt. */
    var felder = [];
    for (var f = 1; f <= SCHIFFE_FELDER; f++) felder.push(f);
    for (var i = felder.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var hin = felder[i]; felder[i] = felder[j]; felder[j] = hin;
    }
    schiffeSpiel.reihe.forEach(function (m, k) { schiffeSpiel.verstecke[m.id] = felder[k]; });

    schiffeAnAlle({ t: "start", reihe: schiffeSpiel.reihe,
                    plaetze: SCHIFFE_FELDER, richter: zustand.ichId });
    /* UND JETZT DAS EINZIGE GEHEIMNIS DES SPIELS: jeder erfaehrt
       sein eigenes Feld — und nur seines. Das geht als persoenliche
       Post an genau EIN Geraet, nicht an alle. */
    schiffeSpiel.reihe.forEach(function (m) {
      var nr = schiffeSpiel.verstecke[m.id];
      if (m.id === zustand.ichId) schiffeEmpfangen({ t: "platz", nr: nr }, zustand.ichId);
      else postSenden(m.id, { art: "spielpost", spiel: { t: "platz", nr: nr } });
    });
  ...
  function schiffeRichter(d, vonId, vonName) {
    ...
    /* RUNDE 88 — „versteck" gibt es nicht mehr. Die Plaetze werden
       beim Start ausgelost (siehe schiffeStarten); kein Mitspieler
       schickt mehr ein Feld an den Schiedsrichter, und deshalb kann
       ihm der Schiedsrichter auch nicht mehr aus Versehen
       verraten, dass eines belegt ist. */
    if (d.t === "schuss") {
  ...
  function schiffeEmpfangen(d, vonId) {
    if (!d || !d.t) return;
    if (d.t === "start") {
      /* RUNDE 88: es gibt keine Versteck-Phase mehr — es wird
         ausgelost, und danach wird geschossen. */
      schiffeStand = { phase: "schiessen", richter: d.richter || vonId,
                       plaetze: Number(d.plaetze) || SCHIFFE_FELDER,
                       reihe: d.reihe || [], tafel: {}, meins: 0,
                       dran: "", dranName: "", raus: [],
                       text: "Die Verstecke sind ausgelost.",
  ...
  function schiffeWahl(nr) {
    ...
    /* RUNDE 88: ein Klick kann nur noch eines bedeuten — schiessen. */
