...
(function () {
  ...
  function alsBildAufsetzen(v) {
    ...
    /* Das Video wird ohnehin nie gezeigt (siehe direkt()), aber ein
       unsichtbares Element kann trotzdem in die Medienauswahl des
       Systems geraten. Also bleibt jeder Weg zu, doppelt genaeht. */
  ...
  /* WIE EIN SZENENFILM AM RAND AUSLÄUFT.
     GEWÜNSCHT: „Spiel mal mit den transparenten Rändern, so dass
     der Chat durchguckt … wie eine Vignette, aber wolkenartig,
     natürlich abgerundet, ohne linear oder auffällig zu sein."
     Eine CSS-Vignette ist immer eine Ellipse, und die erkennt man
     sofort. Die Maske wird deshalb gerechnet — siehe
     werkzeug/bau-wolkenrand.js — und liegt als Bild bereit. Ihre
     Kante schwankt um 8,5 % im Radius; eine Ellipse hätte 0 %. */
  var WOLKENRAND = "filme/rand-wolke.png";
  function wolkigMaskieren(el) {
    var m = "url(" + WOLKENRAND + ")";
  ...
  /* ---- Weg 1: Bild auf eine LEINWAND, Video nur als Zulieferer ----
     GEMELDET, mehrfach: „Dieses Vollbildsymbol ist immer noch nicht
     weg … das müsste wahrscheinlich in ein Format umgewandelt
     werden, wo das nicht mehr antastbar ist."
     Das Format ist nicht der Punkt — der Knopf gehört zum
     <video>-Element, nicht zur Datei. Also wird gar kein Video mehr
     GEZEIGT: es hängt unsichtbar in der Ecke und liefert nur Bilder
     und Ton, und zu sehen ist eine Leinwand. Eine Leinwand hat
     keine Bedienleiste, kein Vollbild, kein Bild-im-Bild, kein
     „Video speichern unter" — sie ist für den Browser ein Bild, das
     sich bewegt. Genau das, was gewünscht war.
     Der Alphakanal bleibt dabei erhalten: drawImage überträgt ihn
     aus einem VP9-WebM mit yuva420p mit. */
  function direkt(quelle, d, s) {
    ...
    c.width = d.breite || 400; c.height = d.hoehe || 712;
    if (d.art === "szene" || d.art === "dunkel") {
      /* Ein Kinobild fuellt die Breite und laeuft am Rand wolkig
         aus, damit es mit der Seite verschmilzt statt darauf zu
         liegen. */
      c.style.cssText = "width:min(100%,96vw);max-height:84vh;object-fit:contain;";
      wolkigMaskieren(c);
    } else {
    ...
    var laeuft = true;
    function malen() {
      if (!laeuft || !document.body.contains(c)) return;
      if (v.readyState >= 2) {
        try {
      ...
      return datei(url, balken.stand).then(function (quelle) {
        ...
        if (d.art === "szene") {
          /* Ein Kinobild braucht Dunkelheit ringsum, sonst kaempft
             es mit dem Chat um die Aufmerksamkeit. Ein „dunkel"-Film
             braucht sie NICHT: er ist an seinen dunklen Stellen
             ohnehin durchsichtig, und genau dort soll der Chat
             durchscheinen. */
