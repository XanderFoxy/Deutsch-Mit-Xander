...
window.LiveChat = (function () {
  ...
  /* =========================================================
     FASSUNG 658 — WENIGER IST SCHNELLER
     ---------------------------------------------------------
     GEMELDET: „die Leitung zu Mark Hawkins steht noch nicht — es wird
     weiter versucht … es dauert extrem lange" und „das Spiel hat
     extrem gehangen".
     NACHGESEHEN: Mark war mit Firefox am Linux-Rechner da (spiel_diagnose).
     Die Liste der Vermittler hatte mit dem eigenen Relais SIEBEN Einträge
     (Cloudflare + sechs öffentliche). Firefox warnt ab fünf, dass die
     Wegesuche dadurch langsam wird, und jeder tote Eintrag (die
     Open-Relay-Server antworten oft nicht mehr) kostet Wartezeit.
     Steht das eigene Relais, genügen deshalb: Cloudflare (das bringt
     STUN, TURN über UDP, TCP und TLS schon mit) und EIN Google-STUN als
     Reserve. Ohne eigenes Relais bleibt alles wie bisher. */
  function mitRelais(server) { return server.concat(NOTVERMITTLER.slice(0, 1)); }
  ...
  function relaisHolen(neu) {
    ...
    relaisLaeuft = relaisRufen({ aktion: "zugang" }).then(function (a) {
      ...
      /* Die eigenen Server ZUERST, dahinter nur noch ein STUN als
         Reserve (Fassung 658, siehe mitRelais). */
      VERMITTLER = mitRelais(a.server);
      return true;
    }).catch(function () {
      relaisLaeuft = null;
      relaisStand.quelle = "oeffentlich";
  ...
  var sitzTausch = {};
  /* FASSUNG 679 — DIE SITZORDNUNG IM SPIEL IST EINE EIGENE.
     ...
     getrennt voneinander behandelt werden". Wer mitspielt und im Spiel
     den Platz wechselt, ändert NUR diese Tabelle. Angewendet wird sie
     nur auf Geräten, die selbst mitspielen, und nur für Mitspieler –
     alle anderen sehen weiter die Chat-Sitzordnung. */
  var spielSitz = {};
  function spielSicht() {
    try { return Boolean(window.DMA_SPIEL && window.DMA_SPIEL.imSpiel && window.DMA_SPIEL.imSpiel()); } catch (e) { return false; }
  }
  function spieltMit(id) {
    try { return Boolean(window.DMA_SPIEL && window.DMA_SPIEL.spieltMit && window.DMA_SPIEL.spieltMit(id)); } catch (e) { return false; }
  }
  /* Das Spiel fragt beim Antworten selbst nach der Lage (meineChatId,
     spielIdVon) – ohne diese Sperre fragten sich beide im Kreis. */
  var spielOrdnungLaeuft = false;
  function spielOrdnungAnwenden(raus) {
    if (spielOrdnungLaeuft || !Object.keys(spielSitz).length) return raus;
    spielOrdnungLaeuft = true;
    try { return spielOrdnungRechnen(raus); } finally { spielOrdnungLaeuft = false; }
  }
  function spielOrdnungRechnen(raus) {
    if (!spielSicht()) return raus;
    Object.keys(spielSitz).sort().forEach(function (id) {
      var ziel = spielSitz[id];
      if (typeof ziel !== "number" || ziel < 0 || ziel >= raus.length || !(id === zustand.ichId || spieltMit(id))) return;
      var a = -1;
      for (var i = 0; i < raus.length; i++) if (raus[i].id === id) a = i;
      if (a < 0 || a === ziel) return;
      var t = raus[a]; raus[a] = raus[ziel]; raus[ziel] = t;
      raus[a].nummer = a + 1; raus[ziel].nummer = ziel + 1;
    });
    return raus;
  }
  ...
  /* =================================================================
     ...
     -----------------------------------------------------------------
     XANDER: „wenn wir uns gleichzeitig mit dem Lasso werfen … dann
     fliegen leere Lassos, und keiner wird gezogen … achte darauf, dass
     sich da nichts verknotet."
     URSACHE: jedes Geraet setzt nach 2,3 s seine EIGENE neue
     Sitzordnung ab — beide gerechnet aus demselben alten Stand. Wer
     zuletzt ankommt, gewinnt, und je nach Laufzeit ist das auf jedem
     Geraet ein anderer: die Sitzreihe laeuft auseinander.
     JETZT gilt eine Vorfahrt, die JEDES Geraet gleich ausrechnet: wer
     die kleinere Kennung hat, zieht; der andere laesst los. Beruehren
     sich zwei Zuege (dieselbe Person, der Werfer des einen ist das
     Ziel des anderen, oder derselbe Zielplatz), bricht der Verlierer
     seinen Platzwechsel ab, bevor er hinausgeht. Die Animation laeuft
     trotzdem — man sieht, dass er es versucht hat. */
  var offeneZuege = [];
  function zugVorfahrt(n) {
    ...
    offeneZuege.forEach(function (z) {
      var beruehrt = wen === z.wen || wen === z.werfer || werfer === z.wen
        || (ziel && ziel === z.ziel);
      if (!beruehrt) return;
      if (String(zustand.ichId) > String(n.von || "")) {
        clearTimeout(z.uhr);
        z.weg = true;
      }
    });
  ...
  /* =================================================================
     RUNDE 99 — NUR DAS SCHICKEN, WAS SICH GEAENDERT HAT
     -----------------------------------------------------------------
     XANDER: „zu zweit reisen funktioniert nicht mehr."
     GEFUNDEN: jede Sitzordnung-Nachricht trug die GANZE eigene Tabelle
     (ordnung: sitzTausch) — also auch alte, laengst ueberholte
     Eintraege fuer andere. Beim Reisen zu zweit setzte sich am Ende
     jedes Geraet selbst um, beide fast gleichzeitig; A schickte dabei
     Bs ALTEN Platz mit und B den alten von A. Wer die Nachricht des
     anderen bekam, wurde auf seinen alten Platz zurueckgesetzt — beide
     sprangen zurueck, je nach Laufzeit auch nur einer.
     Jetzt geht nur hinaus, was sich in DIESEM Augenblick aendert. Der
     Empfaenger fuegt es ohnehin nur ein (siehe „sitzplatz" in
     empfangen); die volle Tabelle faehrt weiter im Puls mit, fuer den,
     der neu dazukommt. */
  function sitzSchicken(ids, text) {
    var teil = {};
  ...
  function plaetzeBauen() {
    ...
    return spielOrdnungAnwenden(raus);
  }
  ...
  var AM_PLATZ = {
    ...
    /* FASSUNG 685 — FUNK 84/85: „der Hot Rod braucht eine Wegzeichnung, er kann auch mit unter Transport"
       und „bei den Autos irgendwie noch ein Monstertruck". Beide fahren wie „fahren" (Kette = gemalter Weg). */
    hotrodfahrt:  { wirkung: "hotrodfahrt",  satz: "f\u00e4hrt im Hot Rod zu", emoji: "\ud83c\udfce\ufe0f" },
    monstertruck: { wirkung: "monstertruck", satz: "donnert im Monstertruck zu", emoji: "\ud83d\udefb" },
    /* FASSUNG 685 — FUNK 85: „Feuerwerk: jemanden als Rakete zünden". */
    zuenden: { wirkung: "zuenden", satz: "zündet eine Rakete unter", emoji: "\ud83c\udf86" },
    ...
    /* RUNDE 101 — XANDER (Funk #39): „Ich möchte eine Waschmaschine als
       Profilbild Effekt wo der andere richtig durchgewirbelt wird". */
    waschmaschine: { wirkung: "waschmaschine", satz: "w\u00e4scht und schleudert", emoji: "\ud83e\udee7" },
    ...
    /* RUNDE 88 — XANDER: „dann mit dem Zauberstab auf den Zylinder
       schlaegt und dann ein Kaninchen an seinen Ohren herausholt und
       dann so Beifall geklatscht wird. Vor Kunststueck halt."
       Und: „Der Zauberer mit Kaninchen, das sollte ein
       Profilbild-Effekt sein, keine Reise." Deshalb ein eigener
       Befehl neben „/zylinder", der die REISE ist. */
    kaninchen:  { wirkung: "kaninchen",  satz: "zaubert aus dem Zylinder", emoji: "\ud83d\udc07" },
    ...
    /* RUNDE 101 — Kopf oder Zahl. Was gefallen ist, steht NICHT im
  ...
  var SPRECHBILDER = {
    ...
    bluete2:    "Blüte 2 — die Knospe platzt auf, zwölf Blätter blühen auf und schließen sich wieder",
    ...
    baerohren:  "Bärenohren — rund und honigwarm",
    maul:       "Vampirmaul — der Rahmen öffnet und schließt sich",
    schemen:    "Schemen — es wird dunkel, ein Schatten huscht am Rand",
  ...
  /* =================================================================
     FASSUNG 642 — DER VERLAUF KOMMT IN HAPPEN, NICHT IN EINEM ZUG
     -----------------------------------------------------------------
     GEMELDET (Xander, 25.09.): „Jetzt steht irgendwas im Chat: der
     gemeinsame Verlauf ließ sich nicht laden. Du siehst gerade nur was
     auf diesem Gerät liegt. Kannst du das reparieren und kannst du
     generell das Laden etwas optimieren, weil manchmal dauert die
     Verbindung zwischen Leuten so lange, dass man ewig warten muss, bis
     man miteinander reden kann."

     NACHGESEHEN — drei Dinge kamen hier zusammen:

     1. DAS PROFILBILD FUHR IN JEDER ZEILE MIT. Die Abfrage holte die
        Spalte „bild" — das Profilbild des Schreibers, als Datenadresse
        bis zu 140 000 Zeichen — fuer bis zu ZWANZIGTAUSEND Zeilen. Seit
        dem 18.09. wird die Tabelle wirklich beschrieben (siehe
        serverSichern, farbe_name); eine Woche Unterricht mit Foto im
        Profil sind schnell ein paar tausend Zeilen mal hundert Kilobyte.
        Das ist keine Abfrage mehr, das ist ein Umzug: die Datenbank
        bricht ihn nach ihrer Zeitgrenze ab (statement_timeout), oder
        das Telefon laedt und laedt — und genau in dieser Zeit verstopft
        der Download die Leitung, ueber die gleichzeitig das Gespraech
        aufgebaut werden soll.
     2. EIN FEHLER SAH AUS WIE EIN LEERER RAUM. serverLaden() gab bei
        jedem Fehler still eine leere Liste zurueck. Wer den Verlauf
        holte, konnte nicht unterscheiden zwischen „hier wurde noch
        nichts geschrieben" und „die Abfrage ist gescheitert" — und hat
        nach vier leeren Antworten geklagt, auch in einem Raum, der
        einfach noch leer war.
     3. KEIN ZWEITER VERSUCH GLEICH DANACH. Ging der erste Anlauf
        schief, kam der naechste erst Sekunden spaeter.

     Jetzt:
       * Zuerst nur das juengste Stueck (VERLAUF_ERST Zeilen, ohne
         Profilbild) — das steht nach einem Wimpernschlag da. Der Rest
         kommt danach im Hintergrund in Seiten zu VERLAUF_SEITE Zeilen,
         jede Seite ein eigener, kleiner Zugriff ueber den Index
         (raum, erstellt desc). Zwanzigtausend Zeilen zum Hochscrollen
         bleiben es trotzdem (CHAT_SICHT).
       * Die Profilbilder einmal je PERSON, nicht einmal je Zeile.
       * Ein Fehler ist ein Fehler (die Zusage wird abgelehnt), eine
         leere Liste ist ein leerer Raum.
       * Jeder Zugriff hat eine Zeitgrenze, damit nichts ewig haengt.
     ================================================================= */
  var VERLAUF_ERST = 300;             // so viele Zeilen stehen sofort da
  var VERLAUF_SEITE = 2000;           // so viele je Nachhol-Zugriff
  var VERLAUF_ZEITGRENZE = 12000;     // laenger wartet kein Zugriff
  var BILDER_ERST = 40;               // Chatbilder im ersten Zugriff
  var PROFILBILDER_HOECHST = 16;      // Personen je Nachhol-Runde
  var VERLAUF_SPALTEN = "id,raum,autor,name,text,art,farbe,farbe_name,quelle_id,erstellt";
  var profilbildJe = {};              // Konto -> juengstes Profilbild

  /* Eine Abfrage an die Tabelle, als ehrliche Zusage: Daten oder
     Ablehnung, und nie laenger als VERLAUF_ZEITGRENZE. */
  function tabellenFrage(bau) {
    return new Promise(function (ok, schief) {
      var abbruch = null, q;
      try {
        q = bau();
        if (q && typeof q.abortSignal === "function" && typeof AbortController !== "undefined") {
          abbruch = new AbortController();
          q = q.abortSignal(abbruch.signal);
        }
      } catch (e) { schief(e); return; }
      var uhr = setTimeout(function () {
        try { if (abbruch) abbruch.abort(); } catch (e) {}
        schief(new Error("Zeitgrenze erreicht"));
      }, VERLAUF_ZEITGRENZE);
      Promise.resolve(q).then(function (a) {
        clearTimeout(uhr);
        if (!a) { schief(new Error("keine Antwort")); return; }
        if (a.error) { schief(a.error); return; }
        ok(a.data || []);
      }, function (f) {
        clearTimeout(uhr);
        schief(f || new Error("abgelehnt"));
      });
    });
  }

  function zeileAusTabelle(r) {
    return {
  ...
  /* Eine Seite des offenen Verlaufs, juengste zuerst geholt, in
     zeitlicher Reihenfolge zurueck. „vor" ist die Zeit der aeltesten
     Zeile der vorigen Seite. */
  function serverSeite(z, raum, vor, anzahl) {
    return tabellenFrage(function () {
      var q = z.from(TISCH).select(VERLAUF_SPALTEN)
        .eq("raum", raum)
        /* Eine gefluesterte Zeile gehoert nicht in den offenen Verlauf —
           sie kommt getrennt, und nur bei den beiden, die sie angeht. */
        .is("an_id", null);
      if (vor) q = q.lt("erstellt", vor);
      return q.order("erstellt", { ascending: false }).limit(anzahl);
    }).then(function (daten) {
      var roh = daten.slice().reverse();
      return {
        zeilen: roh.map(zeileAusTabelle),
        autoren: roh.map(function (r) { return r.autor || ""; }),
        aeltestes: roh.length ? roh[0].erstellt : "",
        voll: daten.length >= anzahl
      };
    });
  }

  /* Das Profilbild einmal je Person — ueber den Index (autor,
     erstellt desc) ist das je Person EIN kleiner Griff. Scheitert er,
     bleibt die Zeile eben ohne Bild; der Verlauf haengt nicht daran. */
  function profilbilderEinsetzen(z, seite) {
    var fehlen = [];
    for (var i = seite.autoren.length - 1; i >= 0 && fehlen.length < PROFILBILDER_HOECHST; i--) {
      var a = seite.autoren[i];
      if (a && !Object.prototype.hasOwnProperty.call(profilbildJe, a) && fehlen.indexOf(a) < 0) fehlen.push(a);
    }
    return Promise.all(fehlen.map(function (a) {
      return tabellenFrage(function () {
        return z.from(TISCH).select("bild").eq("autor", a).is("an_id", null)
          .order("erstellt", { ascending: false }).limit(1);
      }).then(function (d) { profilbildJe[a] = (d[0] && d[0].bild) || ""; },
              function () {});
    })).then(function () {
      seite.zeilen.forEach(function (n, j) {
        var a = seite.autoren[j];
        if (a && profilbildJe[a]) n.bild = profilbildJe[a];
      });
      return seite;
    });
  }

  /* Die Bilder IM Chat — nur die juengsten, in einer eigenen Abfrage.
     quelle_id MUSS mitkommen: die Zeilen tragen die Kennung des
     Absenders, nicht die Nummer der Tabelle — ohne sie fände kein
     einziges Bild seine Zeile. */
  function chatBilderHolen(z, raum, vor, anzahl) {
    return tabellenFrage(function () {
      var q = z.from(TISCH).select("id,quelle_id,bild_im_chat,erstellt")
        .eq("raum", raum)
        .neq("bild_im_chat", "");
      if (vor) q = q.lt("erstellt", vor);
      return q.order("erstellt", { ascending: false }).limit(anzahl);
      ...
      daten.forEach(function (r) {
        if (r.bild_im_chat) nach[r.quelle_id || ("s" + r.id)] = r.bild_im_chat;
      });
      return { nach: nach, anzahl: daten.length,
               aeltestes: daten.length ? daten[daten.length - 1].erstellt : "" };
    });
  }
  function bilderAnwenden(nach, liste) {
    var wieviel = 0;
    if (!nach) return 0;
    (liste || []).forEach(function (n) {
      if (n && n.id && nach[n.id] && !n.bildImChat) {
        n.bildImChat = nach[n.id];
        n.bildWeg = false;
        wieviel += 1;
      }
    });
  ...
  /* Holt den offenen Verlauf eines Raums.
       o.erst      — hoechstens so viele Zeilen (sonst CHAT_SICHT)
       o.bilder    — so viele Chatbilder dazu (0 = keine)
       o.fluestern — die eigenen Flüsterzeilen dazu
     Liefert null, wenn es (noch) keinen Zugang gibt; lehnt ab, wenn
     die Tabelle nicht antwortet. */
  function serverVerlauf(raum, o) {
    var z = angemeldeterZugang();
    if (!z) return Promise.resolve(null);
    o = o || {};
    var ziel = Math.max(1, Math.min(o.erst || CHAT_SICHT, CHAT_SICHT));
    var seiten = [], geholt = 0;
    function seite(vor) {
      return serverSeite(z, raum, vor, Math.min(VERLAUF_SEITE, ziel - geholt)).then(function (s) {
        seiten.unshift(s);
        geholt += s.zeilen.length;
        if (s.voll && s.aeltestes && geholt < ziel) return seite(s.aeltestes);
        return s;
      });
    }
    return seite("").then(function (letzte) {
      var v = { zeilen: [], autoren: [], anzahl: geholt, voll: letzte.voll,
                aeltestes: letzte.aeltestes || "", bilder: null };
      seiten.forEach(function (s) {
        v.zeilen = v.zeilen.concat(s.zeilen);
        v.autoren = v.autoren.concat(s.autoren);
      });
      return profilbilderEinsetzen(z, v);
    }).then(function (v) {
      if (!o.bilder) return v;
      return chatBilderHolen(z, raum, "", o.bilder).then(function (b) {
        v.bilder = b;
        bilderAnwenden(b.nach, v.zeilen);
        return v;
      }, function () { return v; });
    }).then(function (v) {
      if (!o.fluestern) return v;
      /* Auch das Fluestern bekommt eine Zeitgrenze — haengt es, soll der
         offene Verlauf trotzdem erscheinen. */
      var bremse = new Promise(function (fertig) {
        setTimeout(function () { fertig([]); }, VERLAUF_ZEITGRENZE);
      });
      return Promise.race([fluesternLaden(), bremse]).then(function (geheim) {
        v.zeilen = v.zeilen.concat(geheim);
        return v;
      }, function () { return v; });
    });
  }

  /* Der ganze Verlauf in einem Stueck (zum Nachlesen im Archiv und
     fuer die Sonden). Lehnt ab, wenn die Tabelle nicht antwortet. */
  function serverLaden(raum) {
    return serverVerlauf(raum, { bilder: BILDER_ZURUECK, fluestern: true })
      .then(function (v) { return v ? v.zeilen : []; });
  }
  /* =========================================================
  ...
  function verschmelzen(a, b) {
    ...
    /* FASSUNG 642 — „kannst du generell das Laden etwas optimieren"
       Hier suchte JEDE Zeile ihren Zwilling in ALLEN schon genommenen
       Zeilen. Bei zwanzigtausend Zeilen aus der Tabelle plus
       zwanzigtausend aus dem Geraet sind das Hunderte Millionen
       Vergleiche — das Telefon steht dabei sekundenlang still, genau
       beim Betreten, wenn gleichzeitig die Leitung zu den anderen
       aufgebaut werden soll.
       Jetzt: die Kennung ueber ein Verzeichnis (ein Griff), und der
       Vergleich ueber Name, Text und Zeit nur noch rueckwaerts bis
       vier Sekunden zurueck — die Liste ist ja nach Zeit sortiert,
       weiter hinten kann kein Zwilling liegen. Das Ergebnis ist
       dasselbe. */
    var nachKennung = Object.create(null);
    alles.forEach(function (n) {
      var zwilling = null;
      if (n.id) {
        var gleich = nachKennung[n.id];
        if (gleich && gleich.id === n.id) zwilling = gleich;
      }
      if (!zwilling) {
        for (var i = raus.length - 1; i >= 0; i--) {
          var m = raus[i];
          if ((n.zeit || 0) - (m.zeit || 0) >= 4000) break;
          if (m.name === n.name && m.text === n.text
              && Math.abs((m.zeit || 0) - (n.zeit || 0)) < 4000
              && hatBild(m) === hatBild(n)) { zwilling = m; break; }
        }
      }
      if (!zwilling) {
        raus.push(n);
        if (n.id && !nachKennung[n.id]) nachKennung[n.id] = n;
        return;
      }
      ...
      /* Das Profilbild reist nicht mehr in jeder Zeile mit (Fassung
         642) — hat die eine Ausfertigung es, bekommt es die andere. */
      if (!zwilling.bild && n.bild) zwilling.bild = n.bild;
    });
  ...
  function empfangen(n) {
    ...
    if (n.art === "puls") {
      ...
      if (n.sitz && typeof n.sitz === "object") {
        ...
        if (n.spielsitz && typeof n.spielsitz === "object") {
          Object.keys(n.spielsitz).forEach(function (id) {
    ...
    /* Runde 100 — Stadt · Land · Fluss im Raum (siehe slfLiveStarten). */
    if (n.art === "slf") {
      slfLiveEmpfangen(n.slf || {}, n.von);
      return;
    }
    ...
    if (n.art === "tschuess") {
      /* FASSUNG 687 — der Abschied fährt los, solange das Bild noch auf seinem Platz sitzt. */
      if (zustand.leute[n.von] && n.auftritt) auftrittZeigen(n.von, n.auftritt, "raus");
  ...
  /* =========================================================
     FASSUNG 687 — DER EIGENE AUFTRITT
     XANDER: „jeder so einen personalisierten Effekt hat wie er
     ankommt … wenn ich in den Chat komme dass ich dann mit einem
     Ferrari reinkomme". Gewählt im eigenen Platzmenü, gemerkt auf dem
     Gerät, mitgeschickt im „hallo" und im „tschuess". Gezeichnet wird
     in app.js (window.DMA_AUFTRITT).
     ========================================================= */
  var AUFTRITTE = { sportwagen: 1, hotrod: 1, monstertruck: 1, rakete: 1, zauber: 1 };
  function eigenerAuftritt() {
    try { var a = localStorage.getItem("dma_auftritt") || ""; return AUFTRITTE[a] ? a : ""; } catch (e) { return ""; }
  }
  function auftrittZeigen(id, art, richtung) {
    if (!AUFTRITTE[art] || !id) return;
    var los = function () { try { if (window.DMA_AUFTRITT) window.DMA_AUFTRITT(id, art, richtung); } catch (e) {} };
    /* Beim Kommen erst den Platz zeichnen lassen, beim Gehen sofort. */
    if (richtung === "rein") setTimeout(los, 300); else los();
  }
  ...
  /* Den eigenen Verlauf an eine Person schicken. Bilder werden dabei
     ...
     und ein Verlauf mit zwanzig Fotos passt nicht hinein.
     (Seit Fassung 642 nur noch das juengste Stueck — den vollstaendigen
     Text bringt die Tabelle, siehe unten.) */
  /* FASSUNG 642: 24 000 statt 140 000 — dieselbe Groesse, die sich
     bei der Sprachnachricht auf langsamen Leitungen bewaehrt hat
     (PAKET_BYTES). */
  var VERLAUF_PAKET = 24000;
  var VERLAUF_LUFT = 600;
  function verlaufNachreichen(anId) {
    ...
    /* FASSUNG 642: dazu 600 ms Luft — erst gehen Angebot und Kerzen
       fuer die Tonleitung hinaus, dann der Verlauf (siehe unten). */
    var vorsprung = 250 + Math.min(1400, zustand.ichId.charCodeAt(0) % 7 * 180) + VERLAUF_LUFT;
    setTimeout(function () {
      if (verlaufSchonGeschickt[anId]) return;
      verlaufSchonGeschickt[anId] = true;
      /* =============================================================
         FASSUNG 642 — DER VERLAUF DARF DIE LEITUNG NICHT VERSTOPFEN
         -------------------------------------------------------------
         XANDER (25.09.): „manchmal dauert die Verbindung zwischen
         Leuten so lange, dass man ewig warten muss, bis man
         miteinander reden kann."

         NACHGESEHEN: Genau in der Sekunde, in der ein Neuer kommt,
         geht ueber DENSELBEN Kanal auch alles, was die Tonleitung
         braucht — das Angebot, die Antwort und die Kerzen. Und genau
         dann schickte hier jeder im Raum seinen Verlauf hinterher:
         bis zu 140 000 Zeichen, und wenn nach dem Herausnehmen der
         Fotos immer noch zu viel war, die letzten zwanzig Zeilen —
         MIT dem Profilbild in jeder Zeile (je bis zu 140 000 Zeichen),
         also leicht zwei, drei Megabyte in einem Paket. Bei der
         Sprachnachricht war schon gemessen worden, dass so grosse
         Pakete auf langsamen Leitungen verworfen werden (PAKET_BYTES);
         geht dabei eine Kerze mit verloren, steht der Ton erst nach
         dem naechsten Neustart der Wegesuche. Obendrein wurde der
         ganze Verlauf fuer jedes herausgenommene Foto NEU in Text
         verwandelt — bei zwanzigtausend Zeilen stand das Geraet
         dessen, der schon drin sass, dabei sekundenlang still.

         Den ganzen Verlauf holt sich der Neue heute aus der Tabelle
         (verlaufFrischHolen). Hier geht deshalb nur noch das juengste
         Stueck hinaus, in einem Paket von hoechstens VERLAUF_PAKET
         Zeichen, von hinten nach vorn einmal abgezaehlt, ohne
         Profilbilder als Datenadresse. */
      var paket = [], groesse = 2;
      for (var i = zustand.nachrichten.length - 1; i >= 0; i--) {
        var n = zustand.nachrichten[i];
        if (!n || n.art === "fluester" || n.art === "system" || n.art === "kommen") continue;
        var zeile = verlaufZeile(n);
        var laenge = JSON.stringify(zeile).length + 1;
        if (zeile.bildImChat && groesse + laenge > VERLAUF_PAKET) {
          zeile.bildImChat = "";
          zeile.bildWeg = true;
          laenge = JSON.stringify(zeile).length + 1;
        }
        if (laenge > VERLAUF_PAKET) continue;      // eine Riesenzeile allein
        if (groesse + laenge > VERLAUF_PAKET) break;
        groesse += laenge;
        paket.push(zeile);
      }
  ...
  /* Ein Profilbild, das eine Adresse ist (oder ein Aufkleber), reist
     mit; ein Foto als Datenadresse nicht — das kennt der Neue aus der
     Begruessung oder aus der Tabelle. */
  function kurzesProfilbild(b) {
    var s = String(b || "");
  ...
  function verlaufZeile(n) {
    ...
    return zusatzUebernehmen({
      ...
      bild: kurzesProfilbild(n.bild), bildImChat: n.bildImChat || "", farbe: n.farbe || "",
      zeit: n.zeit
    }, n);
  }
  ...
  /* =========================================================
     RUNDE 100 — DIE UEBUNGSPUPPE
     ---------------------------------------------------------
     XANDER (Walkie-Talkie): „Mach mir mal einen Angel-Dummy, dass ich
     testen kann, auch wenn niemand da ist." Und zum Kran und Lasso:
     „Ich brauche einen Dummy zum Testen."
     Die Puppe sitzt NUR auf dem eigenen Geraet auf einem freien Platz
     und laesst sich angeln, mit dem Lasso holen, mit dem Kran heben,
     bespruehen … wie eine echte Person. Damit nichts durcheinander
     geraet, gilt:
       · nur fuer den Betreiber und nur, wenn er allein im Raum ist —
         die anderen haetten sie nicht, und ihre Sitzordnung wuerde
         nicht mehr zu seiner passen;
       · kommt jemand herein, ist sie sofort weg;
       · sie verfaellt nie (kein Puls noetig) und geht nie ins Netz.
     Ihr Bild ist gezeichnet: ein Crashtest-Dummy (gelbes Gesicht mit
     den schwarz-gelben Messmarken) — kein Emoji. */
  var PUPPE_ID = "uebungspuppe";
  var PUPPE_BILD = "data:image/svg+xml;utf8," + encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">'
    + '<rect width="100" height="100" fill="#e9eef2"/>'
    + '<ellipse cx="50" cy="54" rx="31" ry="36" fill="#f2c230" stroke="#a37f10" stroke-width="2"/>'
    + '<circle cx="50" cy="30" r="9" fill="#fff" stroke="#222" stroke-width="1.6"/>'
    + '<path d="M50 21 A9 9 0 0 1 59 30 L50 30 Z M50 39 A9 9 0 0 1 41 30 L50 30 Z" fill="#222"/>'
    + '<ellipse cx="38" cy="54" rx="4.2" ry="3" fill="#2b2b2b"/>'
    + '<ellipse cx="62" cy="54" rx="4.2" ry="3" fill="#2b2b2b"/>'
    + '<path d="M40 72 Q50 77 60 72" fill="none" stroke="#6b5208" stroke-width="2.4" stroke-linecap="round"/>'
    + '<rect x="18" y="50" width="6" height="12" rx="2" fill="#d7a91f"/>'
    + '<rect x="76" y="50" width="6" height="12" rx="2" fill="#d7a91f"/>'
    + '</svg>');
  /* RUNDE 100 — XANDER (Walkie #95/#96): „Ich brauche einen zweiten
     Dummy um das auszutesten" (Fahrrad zu zweit, Angel). Der Knopf
     schaltet jetzt reihum: keine → Puppe → dazu „Dummy" → beide weg.
     „Dummy" ist ein Wort, damit „/heb Dummy 5" eindeutig bleibt. */
  var PUPPE2_ID = "uebungspuppe2";
  var PUPPE2_BILD = PUPPE_BILD.replace(/%23f2c230/g, "%2368b7e8").replace(/%23a37f10/g, "%232a6f9e")
    .replace(/%23d7a91f/g, "%234a9ad0").replace(/%236b5208/g, "%23234f72");
  function istPuppe(id) { return id === PUPPE_ID || id === PUPPE2_ID; }
  function puppeZahl() {
    return (zustand.leute[PUPPE_ID] ? 1 : 0) + (zustand.leute[PUPPE2_ID] ? 1 : 0);
  }
  function puppeDa() { return puppeZahl() > 0; }
  function puppeWeg(grund) {
    if (!puppeDa()) return false;
    [PUPPE_ID, PUPPE2_ID].forEach(function (id) {
      delete zustand.leute[id];
      delete sitzTausch[id];
      delete spielSitz[id];
      if (platzJe[id] != null) delete platzJe[id];
    });
    if (grund) systemZeile(grund);
    melden();
    return true;
  }
  function puppeSetzen(id, name, bild) {
    zustand.leute[id] = { id: id, name: name, strom: null, seit: Date.now(),
      gesehen: 9e15, tonAn: false, bildAn: false, bild: bild, buehne: true,
      sprechbild: "ring", geschlecht: "" };
  }
  function puppeUmschalten(wunsch) {
    if (!zustand.betreiber) return systemZeile("Die Übungspuppe gibt es nur für den Betreiber.");
    if (zustand.lage !== "drin") return systemZeile("Dafür musst du erst im Raum sein.");
    var w = String(wunsch || "").trim().toLowerCase();
    if (w === "weg" || w === "aus") return puppeWeg("🧍 Die Übungspuppen sind weg.") || true;
    var zahl = puppeZahl();
    if (zahl >= 2) return puppeWeg("🧍 Beide Übungspuppen sind weg.");
    var andere = Object.keys(zustand.leute).filter(function (id) { return !istPuppe(id); });
    if (andere.length) {
      return systemZeile("Die Übungspuppe geht nur, wenn du allein im Raum bist — "
        + "die anderen würden sie nicht sehen, und eure Sitzordnung passte nicht mehr zusammen.");
    }
    if (zahl === 0) {
      puppeSetzen(PUPPE_ID, "Puppe", PUPPE_BILD);
      systemZeile("🧍 Die Übungspuppe sitzt jetzt auf einem freien Platz — nur bei dir. "
        + "Tipp auf ihr Bild: Angel, Lasso, Kran, Sprühdose … alles geht. "
        + "Kommt jemand herein, ist sie sofort weg. Nochmal /puppe setzt eine zweite dazu („Dummy“).");
    } else {
      puppeSetzen(PUPPE2_ID, "Dummy", PUPPE2_BILD);
      systemZeile("🧍 Die zweite Übungspuppe „Dummy“ sitzt jetzt auch da — für Zu zweit, Angel, Lasso "
        + "zwischen zwei anderen. Nochmal /puppe nimmt beide ab.");
    }
    melden();
    return true;
  }
  function personMerken(id, name, bild) {
    /* Runde 100 — kommt ein Mensch herein, geht die Puppe. */
    if (!istPuppe(id) && puppeDa() && !zustand.leute[id]) {
      puppeWeg("🧍 Die Übungspuppe ist weg — es ist jemand hereingekommen.");
    }
    if (!zustand.leute[id]) {
  ...
  function wacheStarten() {
    ...
    wacheUhr = setInterval(function () {
      ...
      Object.keys(zustand.leute).forEach(function (id) {
        if (id === zustand.ichId) return;
        if (istPuppe(id)) return;   /* Runde 100: die Puppe hat keine Leitung */
      ...
      Object.keys(brueckeJe).forEach(function (id) {
        ...
        if (zustand.ichId < id) {
          ...
          senden({ art: "hallo", auftritt: eigenerAuftritt(), name: zustand.ichName, tonAn: zustand.tonAn,
                   bildAn: zustand.bildAn, bild: zustand.ichBild, farbe: zustand.farbe, farbeName: zustand.farbeName,
                   seit: zustand.seit, buehne: zustand.buehne,
                   geschlecht: zustand.geschlecht || "", konto: kontoId || "" });
        }
  ...
  var verlaufUhren = [];
  /* =================================================================
     FASSUNG 642 — ERST NOCH ZWEIMAL VERSUCHEN, DANN LEISE SAGEN
     -----------------------------------------------------------------
     GEMELDET (Xander, 25.09.): „Jetzt steht irgendwas im Chat: der
     gemeinsame Verlauf ließ sich nicht laden. Du siehst gerade nur was
     auf diesem Gerät liegt."

     Die Zeile kam, sobald vier Anlaeufe eine LEERE Liste gebracht
     hatten — und leer war sie bei jedem Fehler, aber eben auch in
     jedem Raum, in dem noch nie etwas geschrieben wurde. Dort war die
     Meldung schlicht falsch. Und dort, wo die Tabelle wirklich nicht
     antwortete, lag es an der viel zu grossen Abfrage (siehe
     serverVerlauf) — ein zweiter Versuch gleich hinterher gab es nicht.

     Jetzt:
       * Jeder Anlauf versucht es bis zu dreimal (Pausen VERLAUF_PAUSEN
         dazwischen, jede laenger als die vorige). Ein kurzer Wackler
         ist damit weg, bevor ihn jemand bemerkt.
       * Eine leere Antwort ist eine ANTWORT: der Raum ist leer, und
         das ist in Ordnung — kein Hinweis.
       * Gesagt wird es nur, wenn WIRKLICH alle Anlaeufe gescheitert
         sind (auch die Nachfass-Termine), und dann dezent als
         Einblendung statt als Zeile, die im Verlauf stehen bleibt.
         Danach wird im Hintergrund weiter versucht; klappt es, steht
         der Verlauf einfach da.
     ================================================================= */
  var VERLAUF_PAUSEN = [700, 2000];              // zwei weitere Versuche
  var VERLAUF_NACHFASSEN = [2500, 8000, 20000];  // weitere Anlaeufe
  var VERLAUF_SPAETER = [60000, 180000];         // nach dem Hinweis
  var verlaufUhrenOffen = 0;
  var verlaufFehlAnlaeufe = 0;
  var verlaufLauf = null;
  var verlaufLaufRaum = "";
  var verlaufHierGeklagt = false;
  ...
  function verlaufNachfassenPlanen(raum, liste) {
    liste.forEach(function (ms) {
      verlaufUhrenOffen += 1;
      verlaufUhren.push(setTimeout(function () {
        verlaufUhrenOffen = Math.max(0, verlaufUhrenOffen - 1);
        if (verlaufVomServer) return;
        if (zustand.raum !== raum) return;
        /* Laeuft noch einer, wird sein Ergebnis abgewartet — er zaehlt
           fuer diesen Termin mit. */
        if (verlaufLauf && verlaufLaufRaum === raum) return;
        verlaufFrischHolen(false);
      }, ms));
    });
  }

  /* Ein Anlauf: bis zu drei Versuche mit wachsender Pause. */
  function verlaufVersuchen(raum) {
    var nr = 0;
    function einmal() {
      verlaufVersuche += 1;
      return serverVerlauf(raum, { erst: VERLAUF_ERST, bilder: BILDER_ERST, fluestern: true })
        .then(null, function (fehler) {
          if (nr >= VERLAUF_PAUSEN.length || zustand.raum !== raum) throw fehler;
          var pause = VERLAUF_PAUSEN[nr];
          nr += 1;
          return new Promise(function (weiter) { setTimeout(weiter, pause); }).then(einmal);
        });
    }
    return einmal();
  }

  function verlaufKlagen(raum) {
    if (verlaufGeklagt) return;
    verlaufGeklagt = true;
    verlaufHierGeklagt = true;
    var satz = "Der gemeinsame Verlauf kommt gerade nicht durch — du siehst, "
      + "was auf diesem Gerät liegt. Es wird im Hintergrund weiter versucht.";
    if (typeof hinweisRuf === "function") hinweisZeigen(satz);
    else fluechtigeZeile("ℹ️ " + satz, 12000);
    verlaufNachfassenPlanen(raum, VERLAUF_SPAETER);
  }

  function eigeneMarkieren(liste) {
    liste.forEach(function (n) { n.eigen = n.von && n.von === zustand.ichId; });
  }

  /* Was nach dem ersten Stueck noch fehlt, kommt im Hintergrund:
     aeltere Seiten zum Hochscrollen und die weiteren Chatbilder. Das
     Betreten wartet darauf nicht, und jede Seite ist ein eigener,
     kleiner Zugriff. */
  /* Nur beim ERSTEN Holen eines Raums geht es ganz nach hinten. Jedes
     weitere Holen (Zurueckkommen auf die Seite, Nachfassen) schliesst
     nur die Luecke bis zu dem, was schon vom Server da ist — sonst
     wuerden bei jedem Zurueckkommen zwanzigtausend Zeilen neu geholt. */
  var verlaufRestFuer = "";
  var verlaufBekanntBis = 0;
  function verlaufRestHolen(raum, v, bis) {
    var z = angemeldeterZugang();
    if (!z || !v) return Promise.resolve(0);
    var geholt = v.anzahl || 0, dazu = 0, geaendert = false;
    var reichtBis = function (zeilen) {
      return Boolean(bis && zeilen.length && (zeilen[0].zeit || 0) <= bis);
    };
    function seite(vor) {
      if (zustand.raum !== raum || geholt >= CHAT_SICHT) return Promise.resolve();
      return serverSeite(z, raum, vor, Math.min(VERLAUF_SEITE, CHAT_SICHT - geholt))
        .then(function (s) { return profilbilderEinsetzen(z, s); })
        .then(function (s) {
          if (zustand.raum !== raum) return;
          geholt += s.zeilen.length;
          dazu += s.zeilen.length;
          if (s.zeilen.length) {
            geaendert = true;
            eigeneMarkieren(s.zeilen);
            zustand.nachrichten = verschmelzen(s.zeilen, zustand.nachrichten);
            if (v.bilder) bilderAnwenden(v.bilder.nach, zustand.nachrichten);
            melden();
          }
          if (s.voll && s.aeltestes && !reichtBis(s.zeilen)) return seite(s.aeltestes);
        });
    }
    var ersteOffene = v.zeilen.slice(0, v.anzahl || 0);
    var zeilenLauf = (v.voll && v.aeltestes && !reichtBis(ersteOffene))
      ? seite(v.aeltestes) : Promise.resolve();
    return zeilenLauf.then(function () {
      if (zustand.raum !== raum) return;
      if (bis) return;                 // die aelteren Bilder kamen schon beim ersten Mal
      if (!v.bilder || v.bilder.anzahl < BILDER_ERST || !v.bilder.aeltestes) return;
      return chatBilderHolen(z, raum, v.bilder.aeltestes, Math.max(0, BILDER_ZURUECK - BILDER_ERST))
        .then(function (b) {
          if (zustand.raum !== raum) return;
          if (bilderAnwenden(b.nach, zustand.nachrichten)) geaendert = true;
        });
    }).then(null, function (fehler) {
      try { console.warn("Klassenzimmer: aelterer Verlauf nicht vollstaendig geholt —", fehler); } catch (e) {}
    }).then(function () {
      if (geaendert && zustand.raum === raum) { chatSichern(); melden(); }
      return dazu;
    });
  }
  ...
  function verlaufFrischHolen(ersterAnlauf) {
    ...
    if (ersterAnlauf) {
      ...
      verlaufVersuche = 0;
      verlaufFehlAnlaeufe = 0;
      verlaufHierGeklagt = false;
      verlaufUhrenStoppen();
      /* Nachfassen, falls die Anmeldung noch nicht stand oder die
         Tabelle nicht antwortete. Die Uhren laufen nur weiter, solange
         nichts angekommen ist. */
      verlaufNachfassenPlanen(raum, VERLAUF_NACHFASSEN);
    }
    if (verlaufLauf && verlaufLaufRaum === raum) return verlaufLauf;
    if (!angemeldeterZugang()) {
      /* Die Anmeldung (backend.js) steht noch nicht — das ist kein
         Fehler, das Nachfassen holt es gleich nach. */
      verlaufVersuche += 1;
      return Promise.resolve(false);
    }
    verlaufLaufRaum = raum;
    var lauf = verlaufLauf = verlaufVersuchen(raum).then(function (v) {
      if (verlaufLauf === lauf) verlaufLauf = null;
      if (zustand.raum !== raum) return false;
      if (!v) return false;
      /* Die Tabelle hat geantwortet — auch wenn sie nichts hatte. */
      verlaufVomServer = true;
      verlaufUhrenStoppen();
      if (verlaufHierGeklagt) {
        verlaufHierGeklagt = false;
        if (typeof hinweisRuf === "function") hinweisZeigen("Der gemeinsame Verlauf ist jetzt da.");
      }
      var vomServer = v.zeilen;
      if (vomServer.length) {
        eigeneMarkieren(vomServer);
      ...
      var bis = verlaufRestFuer === raum ? verlaufBekanntBis : 0;
      if (verlaufRestFuer !== raum) { verlaufRestFuer = raum; verlaufBekanntBis = 0; }
      v.zeilen.slice(0, v.anzahl || 0).forEach(function (n) {
        if ((n.zeit || 0) > verlaufBekanntBis) verlaufBekanntBis = n.zeit || 0;
      });
      verlaufRestHolen(raum, v, bis);
      return vomServer.length > 0;
    }, function (fehler) {
      if (verlaufLauf === lauf) verlaufLauf = null;
      if (zustand.raum !== raum) return false;
      verlaufFehlAnlaeufe += 1;
      try { console.warn("Klassenzimmer: gemeinsamer Verlauf nicht geholt —", fehler); } catch (e) {}
      /* Nur wenn wirklich nichts mehr aussteht: kein Nachfass-Termin
         mehr offen, und nie etwas vom Server bekommen. */
      if (!verlaufVomServer && !verlaufGeklagt && verlaufUhrenOffen === 0) verlaufKlagen(raum);
      return false;
    });
    return lauf;
  }
  ...
  function verlassen(wohin) {
    ...
    zustand.auftrittRaum = "";
    senden(zielRaum
      ? { art: "tschuess", wohin: zielRaum, wohinName: raumKlartext(zielRaum), auftritt: eigenerAuftritt() }
      : { art: "tschuess", auftritt: eigenerAuftritt() });
    if (kanal) {
  ...
  function noteGeben(id, zahl, wofuer) {
    ...
    /* FUNK 95 — die Note zaehlt auch im Spiel; der Server prueft, dass
       wirklich der Lehrer sie gibt. */
    if (id !== zustand.ichId) {
      try { if (window.DMA_SPIEL && window.DMA_SPIEL.lehrerNote) window.DMA_SPIEL.lehrerNote(id, zahl); } catch (e) {}
    }
    /* =========================================================
  ...
  function platzNehmen(nummer) {
    ...
    /* FUNK 89: „du kannst nicht reisen oder du kannst nicht wechseln du bist kaputt" */
    if (spielGesperrt()) return { ok: false, warum: window.DMA_SPIEL.gesperrtGrund() };
    var n = Number(nummer);
  ...
  function platzTauschenMit(id) {
    ...
    if (spielSicht()) {
      /* Im Spiel: nur die Spiel-Sitzordnung. Wer nicht mitspielt, rückt
         dabei nur für die Mitspieler auf meinen alten Platz. */
  ...
  function aufgabeFreiStellen(roh) {
    ...
    if (!text) {
      ...
      /* =========================================================
         RUNDE 88 — „ICH WEISS IMMER NOCH NICHT, WAS DAS SEIN SOLL"
         ---------------------------------------------------------
         XANDER, woertlich: „Die Aufgabe hatte noch keine Funktion,
         immer noch nicht — ich weiss immer noch nicht, was das sein
         soll."
         Hier stand bisher ein einziger Beispielsatz. Der erklaert die
         FREIE Aufgabe — aber das Aufgaben-Modul hat acht Arten, und
         jede hat ihren eigenen Befehl. Wer „/aufgabe" tippte, sah
         also eine von acht und keinen Hinweis auf die anderen
         sieben. Genau das war der Grund.
         Jetzt oeffnet „/aufgabe" ohne Text die Tafel in der
         Oberflaeche (app.js, lcAufgabenWaehler): alle Arten, jede
         mit einem Satz, was sie tut, mit Beispiel und damit, als was
         die Note zaehlt. Ohne Oberflaeche — Sonde, Kopfrechner —
         bleibt die Zeile von frueher stehen. */
      var tafelAuf = false;
      try { tafelAuf = Boolean(window.DMA_AUFGABENWAHL && window.DMA_AUFGABENWAHL()); }
      catch (e) { tafelAuf = false; }
      if (tafelAuf) return true;
      return systemZeile("So geht es:  /aufgabe Schreib einen Satz mit „weil“"
        + "  — und  /aufgabe  ohne Text beendet sie wieder."
        + "\nMehr Arten:  /satz  /wort  /betonung  /raten  /kontexter"
        + "  /artikel  /begriff");
    }
  ...
  /* =================================================================
     ...
     XANDER dazu: „diese Reihenfolge muss einem logischen Prinzip
     folgen, nach der Reihe der Anmeldungen im Chat … weil, wenn die
     Leute staendig ihre Plaetze tauschen … nicht, dass das System
     dann zaehlt, dass derjenige der Dritte ist und der Achte."

     RUNDE 88 — NIEMAND SUCHT SICH MEHR SEIN VERSTECK AUS.
     XANDER hat den Fehler selbst gefunden: „Das Schiffe versenken
     macht im Uebrigen auch keinen Sinn. Da muessen wir uns etwas
     ueberlegen, wie wir das spielen koennen, denn wenn sich ein
     Zweiter einen Platz aussucht, dann darf er nicht rausfinden,
     dass ein Platz besetzt ist, weil dann wuerde er den ja
     anklicken, um ihn zu versenken. Deswegen musst du das eher mit
     Zufallszahlen loesen, dass einfach jeder zufaellig gesetzt wird
     und die anderen das eben nicht wissen, wo sie hingesetzt sind."

     Er hat recht, und es war wirklich so: wer ein besetztes Feld
     anklickte, bekam „Dort versteckt sich schon jemand" zurueck —
     das war eine Auskunft ueber ein fremdes Versteck, also genau
     das, was das Spiel kaputtmacht. Jetzt lost der Schiedsrichter
     die Plaetze aus (Fisher-Yates ueber alle sechzehn Felder) und
     sagt jedem NUR SEINEN EIGENEN, als persoenliche Post. Niemand
     klickt mehr etwas an, um sich zu verstecken, also kann auch
     niemand etwas erfragen.

     SECHZEHN FELDER, nicht acht — auch das seine Rechnung: „weil
     wir sind ja maximal acht Leute, und damit wir ein bisschen
     Verteilung machen koennen … dass da vielleicht ploetzlich
     sechzehn sind." Bei acht Feldern und acht Spielern waere jedes
     Feld belegt; bei sechzehn ist hoechstens jedes zweite.
     ================================================================= */
  var SCHIFFE_FELDER = 16;    /* so viele Felder hat das Spielbrett */
  ...
  /* RUNDE 88 — DER SCHIEDSRICHTER SIEHT SEIN EIGENES SPIEL.
     XANDER: „hast du das ueberprueft, dass es auf jedem Geraet von
     allen nacheinander gespielt werden kann" und „jeder soll auf
     jeder Seite immer alles sehen und alles hoeren."
     GEFUNDEN: „senden" ist ein Rundruf, und „empfangen" wirft die
     eigene Post weg („die eigene Post nicht lesen"). Damit kam beim
     Spielleiter NICHTS an: nicht, dass er dran ist, nicht, wer
     getroffen wurde, nicht das Spielende. Nur der Aufbau des
     Brettes war von Hand nachgereicht. Jetzt legt jeder Rundruf
     auch auf dem eigenen Geraet an — ein Weg fuer alle. */
  function schiffeAnAlle(d) {
    senden({ art: "spiel", spiel: d });
    schiffeEmpfangen(d, zustand.ichId);
  }
  ...
  /* =================================================================
     RUNDE 100 — STADT · LAND · FLUSS IM KLASSENZIMMER
     -----------------------------------------------------------------
     XANDER (Walkie #87): „Stadt Land Fluß ist kein eigenes Spiel im
     Livestream wie geplant. Es läßt mich immer den Livestream
     verlassen. Das soll so nicht sein." — und zum Lehrer-Panel:
     „Andere sehen es nicht."
     Bisher sprang der Knopf nur auf die Lernseite; dort laeuft das
     Spiel ueber Herausforderungen, nicht im Raum. Jetzt gibt es eine
     Runde IM Raum, fuer alle, die da sind:
       · wer startet (/slf), ist Spielleiter: Buchstabe und Spalten
         gehen an alle, jedes Geraet zeigt unter der Buehne sein Blatt;
       · „Stopp!" gibt das eigene Blatt ab, alle anderen haben noch
         3 Sekunden; sonst endet die Runde nach der Zeit;
       · die Blaetter gehen als persoenliche Post an den Spielleiter,
         er wertet mit derselben Wertung wie das grosse Spiel
         (slfWerten in app.js: 20 nur du, 10 eigenes Wort, 5 gleiches)
         und schickt das Ergebnis an alle.
     ================================================================= */
  var slfLive = null;       // auf jedem Geraet: die laufende Runde
  var slfLeitung = null;    // nur beim Spielleiter: die eingegangenen Blaetter
  var slfLiveUhr = 0;
  function slfLiveZeichnen() {
    try { if (typeof window.DMA_SLF_LIVE === "function") window.DMA_SLF_LIVE(slfLive); } catch (e) {}
  }
  function slfLiveAnAlle(d) {
    senden({ art: "slf", slf: d });
    /* Die eigene Rundsendung hoert man nicht — also hier selbst. */
    slfLiveEmpfangen(d, zustand.ichId);
  }
  function slfLiveStarten(roh) {
    if (zustand.lage !== "drin") return systemZeile("Dafür musst du erst im Raum sein.");
    var r = String(roh || "").trim().toLowerCase();
    if (r === "stopp" && slfLive && slfLive.phase === "schreiben") return slfLiveStopp();
    if (r === "aus" || r === "ende") {
      if (!slfLive) return systemZeile("Es läuft gerade kein Stadt · Land · Fluss.");
      slfLiveAnAlle({ t: "aus", runde: slfLive.runde, wer: zustand.ichName });
      return true;
    }
    if (slfLive && (slfLive.phase === "schreiben" || slfLive.phase === "warten")) {
      return systemZeile("Es läuft schon eine Runde Stadt · Land · Fluss.");
    }
    var hilfe = window.DMA_SLF || {};
    var buchstaben = hilfe.buchstaben || "abcdefghijklmnoprstuvwz".split("");
    var b = /^[a-z]$/.test(r) ? r : buchstaben[Math.floor(Math.random() * buchstaben.length)];
    var spalten = null;
    try { spalten = typeof hilfe.spaltenJetzt === "function" ? hilfe.spaltenJetzt() : null; } catch (e) {}
    if (!spalten || !spalten.length) {
      spalten = [{ id: "stadt", name: "Stadt" }, { id: "land", name: "Land" }, { id: "fluss", name: "Fluss" }];
    }
    spalten = spalten.slice(0, 6).map(function (x) { return { id: String(x.id), name: String(x.name) }; });
    var dauer = Number(hilfe.dauer) || 120;
    var mit = spielReihe().filter(function (x) { return !istPuppe(x.id); })
      .map(function (x) { return { id: x.id, name: x.name }; });
    var runde = Date.now();
    slfLeitung = { runde: runde, blaetter: {}, erwartet: mit.map(function (x) { return x.id; }),
                   namen: {}, frist: 0, fertig: false };
    mit.forEach(function (x) { slfLeitung.namen[x.id] = x.name; });
    slfLiveAnAlle({ t: "start", runde: runde, buchstabe: b, spalten: spalten, dauer: dauer,
                    leiter: zustand.ichId, leiterName: zustand.ichName, mit: mit });
    anAlle("aktion", zustand.ichName + " startet Stadt · Land · Fluss — Buchstabe „"
      + b.toUpperCase() + "“, " + Math.round(dauer / 60 * 10) / 10 + " Minuten. Das Blatt steht unter der Bühne.");
    return true;
  }
  function slfLeitungFrist(ms) {
    if (!slfLeitung || slfLeitung.fertig) return;
    clearTimeout(slfLeitung.frist);
    slfLeitung.frist = setTimeout(slfLiveWerten, ms);
  }
  function slfLiveEmpfangen(d, von) {
    if (!d || !d.t) return;
    if (d.t === "start") {
      clearTimeout(slfLiveUhr);
      var dauer = Math.max(20, Math.min(600, Number(d.dauer) || 120));
      slfLive = { runde: d.runde, buchstabe: String(d.buchstabe || "a").slice(0, 1),
                  spalten: Array.isArray(d.spalten) ? d.spalten.slice(0, 6) : [],
                  endeUm: Date.now() + dauer * 1000, leiter: d.leiter, leiterName: d.leiterName || "",
                  mit: Array.isArray(d.mit) ? d.mit : [], phase: "schreiben", blatt: {},
                  abgegeben: false, stoppVon: "" };
      slfLiveUhr = setTimeout(slfLiveAbgeben, dauer * 1000);
      /* Der Spielleiter wartet 8 s laenger — so viel Zeit hat die Post. */
      if (d.leiter === zustand.ichId) slfLeitungFrist(dauer * 1000 + 8000);
      slfLiveZeichnen();
      return;
    }
    if (!slfLive || d.runde !== slfLive.runde) return;
    if (d.t === "stopp") {
      if (slfLive.leiter === zustand.ichId) slfLeitungFrist(3000 + 6000);
      if (slfLive.phase !== "schreiben") return;
      slfLive.stoppVon = String(d.wer || "Jemand");
      var bis = Date.now() + 3000;
      if (bis < slfLive.endeUm) {
        slfLive.endeUm = bis;
        clearTimeout(slfLiveUhr);
        slfLiveUhr = setTimeout(slfLiveAbgeben, 3000);
      }
      slfLiveZeichnen();
      return;
    }
    if (d.t === "ergebnis") {
      clearTimeout(slfLiveUhr);
      slfLive.phase = "ergebnis";
      slfLive.ergebnis = { namen: d.namen || [], zellen: d.zellen || {}, punkte: d.punkte || [] };
      slfLiveZeichnen();
      return;
    }
    if (d.t === "aus") {
      clearTimeout(slfLiveUhr);
      if (slfLeitung) { clearTimeout(slfLeitung.frist); slfLeitung = null; }
      slfLive = null;
      systemZeile("Stadt · Land · Fluss ist beendet" + (d.wer ? " (" + d.wer + ")" : "") + ".");
      slfLiveZeichnen();
    }
  }
  function slfLiveAbgeben() {
    if (!slfLive || slfLive.abgegeben || slfLive.phase !== "schreiben") return;
    slfLive.abgegeben = true;
    slfLive.phase = "warten";
    clearTimeout(slfLiveUhr);
    var blatt = {};
    slfLive.spalten.forEach(function (sp) {
      blatt[sp.id] = String(slfLive.blatt[sp.id] || "").trim().slice(0, 40);
    });
    var paket = { t: "blatt", runde: slfLive.runde, blatt: blatt };
    if (slfLive.leiter === zustand.ichId) slfLiveBlatt(paket, zustand.ichId, zustand.ichName);
    else postSenden(slfLive.leiter, { art: "slfpost", slf: paket });
    slfLiveZeichnen();
  }
  function slfLiveStopp() {
    if (!slfLive || slfLive.phase !== "schreiben") return false;
    var runde = slfLive.runde;
    slfLiveAbgeben();
    slfLiveAnAlle({ t: "stopp", runde: runde, wer: zustand.ichName });
    return true;
  }
  /* Nur beim Spielleiter: ein Blatt kommt an. */
  function slfLiveBlatt(d, von, name) {
    if (!slfLeitung || slfLeitung.fertig || !d || d.runde !== slfLeitung.runde) return;
    var blatt = {};
    Object.keys(d.blatt || {}).slice(0, 8).forEach(function (k) {
      blatt[String(k).slice(0, 20)] = String(d.blatt[k] || "").slice(0, 40);
    });
    slfLeitung.blaetter[von] = { name: String(name || slfLeitung.namen[von] || "Gast").slice(0, 40), blatt: blatt };
    var fehlt = slfLeitung.erwartet.filter(function (id) { return !slfLeitung.blaetter[id]; });
    if (!fehlt.length) slfLiveWerten();
  }
  function slfLiveWerten() {
    if (!slfLeitung || slfLeitung.fertig || !slfLive) return;
    slfLeitung.fertig = true;
    clearTimeout(slfLeitung.frist);
    var ids = Object.keys(slfLeitung.blaetter);
    var namen = ids.map(function (id) { return slfLeitung.blaetter[id].name; });
    var werten = (window.DMA_SLF || {}).werten;
    var erg = { punkte: ids.map(function () { return 0; }), zellen: {} };
    try {
      if (typeof werten === "function" && ids.length) {
        erg = werten({ buchstabe: slfLive.buchstabe, spalten: slfLive.spalten },
                     ids.map(function (id) { return { blatt: slfLeitung.blaetter[id].blatt }; }));
      }
    } catch (e) {}
    slfLiveAnAlle({ t: "ergebnis", runde: slfLeitung.runde, namen: namen,
                    zellen: erg.zellen, punkte: erg.punkte });
    if (!ids.length) {
      anAlle("aktion", "Stadt · Land · Fluss: kein Blatt ist angekommen.");
      return;
    }
    var rang = ids.map(function (id, i) { return { id: id, name: namen[i], p: erg.punkte[i] || 0 }; })
      .sort(function (a, b) { return b.p - a.p; });
    anAlle("aktion", "Stadt · Land · Fluss „" + slfLive.buchstabe.toUpperCase() + "“: "
      + rang.map(function (x, i) { return (i + 1) + ". " + x.name + " " + x.p; }).join(" · "));
    /* Punkte wie bei Schiffe versenken: ein Zehntel, hoechstens 10. */
    rang.forEach(function (x) {
      var wieviel = Math.min(10, Math.round(x.p / 10));
      if (wieviel <= 0) return;
      var grund = "Stadt · Land · Fluss";
      if (x.id === zustand.ichId) {
        if (typeof zustand.punkteRuf === "function") {
          try { zustand.punkteRuf(wieviel, grund); } catch (e) {}
        }
      } else if (!istPuppe(x.id)) {
        postSenden(x.id, { art: "punkte", wieviel: wieviel, grund: grund });
      }
    });
  }
  ...
  function schiffeStarten(roh) {
    ...
    if (reihe.length > SCHIFFE_FELDER) reihe = reihe.slice(0, SCHIFFE_FELDER);
    /* =================================================================
       RUNDE 92 — DAS VERSTECKEN KOMMT ZURUECK, ABER LAUTLOS
       -----------------------------------------------------------------
       XANDER: „Das Schiffe versenken sollte 16 Plaetze haben, und es
       darf nicht sein, dass man die Animation desjenigen sieht, der
       sich versteckt, denn dann wird er direkt verraten. In dem Moment
       darf man nichts hoeren. Man darf ihn nur sprechen hoeren, aber
       man soll keine Animation an dem Platz sehen, wo das passiert. In
       dem Modus muss man wirklich versteckt bleiben koennen. Man muss
       ...
       In Runde 88 wurden die Plaetze ausgelost — aus Angst, der
       Schiedsrichter koenne verraten, welches Feld belegt ist. Das
       loest das Problem falsch herum: es nimmt ihm die Wahl.
       RICHTIG IST: man waehlt selbst, und die Wahl geht als
       PERSOENLICHE POST nur an den Schiedsrichter. An alle geht
       ausschliesslich eine ZAHL — wie viele schon fertig sind. Wer
       wo liegt, weiss ausser ihm selbst niemand, auch nicht, ob ein
       Feld ueberhaupt belegt ist.

       Und waehrend des ganzen Spiels ist die Sitzreihe still: das
       Klassenzimmer zeigt dort keine Sprech- und keine
       Effektanimation mehr (siehe „lc-schiffe-an" in korrekturen.css).
       Hoeren kann man einander weiter — nur sehen soll man nichts.
       ================================================================= */
    schiffeSpiel = {
  ...
  /* Der Schiedsrichter nimmt ein Versteck entgegen — und verraet es
     niemandem. Zurueck geht nur an DEN EINEN die Bestaetigung, an
     alle anderen hoechstens die Zahl der Fertigen. */
  function schiffeVersteckAn(nr, vonId) {
    if (!schiffeSpiel || schiffeSpiel.phase !== "verstecken") return;
    nr = Number(nr) || 0;
    if (nr < 1 || nr > SCHIFFE_FELDER) return;
    var dabei = schiffeSpiel.reihe.some(function (m) { return m.id === vonId; });
    if (!dabei) return;
    /* Zwei duerfen nicht im selben Feld liegen. Wer ein belegtes
       waehlt, bekommt ein „belegt" zurueck — und zwar NUR er. Dass
       er damit ein fremdes Feld kennt, ist unvermeidlich; deshalb
       steht dort kein Name, und es ist genau die Auskunft, die er
       auch beim Hinsetzen im echten Raum haette. */
    var frei = true;
    Object.keys(schiffeSpiel.verstecke).forEach(function (id) {
      if (id !== vonId && schiffeSpiel.verstecke[id] === nr) frei = false;
    });
    if (!frei) {
      schiffePost(vonId, { t: "belegt", nr: nr });
      return;
    }
    schiffeSpiel.verstecke[vonId] = nr;
    schiffePost(vonId, { t: "platz", nr: nr });
    var fertig = Object.keys(schiffeSpiel.verstecke).length;
    schiffeAnAlle({ t: "wartet", fertig: fertig, von: schiffeSpiel.reihe.length });
    if (fertig >= schiffeSpiel.reihe.length) schiffeAlleDa(false);
  }
  ...
  /* Alle da (oder die Frist ist um): wer noch kein Feld hat, bekommt
     ein freies zugelost — sonst kann er nicht mitspielen. Dann der
     Countdown, und dann geht es los. */
  function schiffeAlleDa(frist) {
    if (!schiffeSpiel || schiffeSpiel.phase !== "verstecken") return;
    if (schiffeSpiel.frist) { clearTimeout(schiffeSpiel.frist); schiffeSpiel.frist = 0; }
    var belegt = {};
    Object.keys(schiffeSpiel.verstecke).forEach(function (id) {
      belegt[schiffeSpiel.verstecke[id]] = true;
    });
    schiffeSpiel.reihe.forEach(function (m) {
      if (schiffeSpiel.verstecke[m.id]) return;
      var frei = [];
      for (var f = 1; f <= SCHIFFE_FELDER; f++) if (!belegt[f]) frei.push(f);
      if (!frei.length) return;
      var nr = frei[Math.floor(Math.random() * frei.length)];
      belegt[nr] = true;
      schiffeSpiel.verstecke[m.id] = nr;
      schiffePost(m.id, { t: "platz", nr: nr, gelost: true });
    });
    schiffeSpiel.phase = "countdown";
    schiffeAnAlle({ t: "countdown", sek: 3,
                    grund: frist ? "Die Zeit ist um" : "Alle sind versteckt" });
    setTimeout(function () {
      if (!schiffeSpiel) return;
      schiffeSpiel.phase = "schiessen";
      schiffeLos();
    }, 3200);
  }
  ...
  function schiffeRichter(d, vonId, vonName) {
    ...
    /* RUNDE 92 — „versteck" gibt es wieder: jeder sucht sich seinen
       Platz selbst aus, und nur der Schiedsrichter erfaehrt ihn. */
    if (d.t === "versteck") { schiffeVersteckAn(d.nr, vonId); return; }
    if (d.t === "schuss") {
  ...
  function schiffeSchuss(nr, vonId, vonName) {
    /* RUNDE 100 — XANDER (Walkie-Talkie): „Die unteren Reihen sind
       ueberhaupt nicht anwaehlbar."
       GEFUNDEN: hier stand „nr > PLAETZE" — und PLAETZE ist 8 (die
       Sitzreihe). Verstecken durfte man sich auf allen 16 Feldern
  ...
  function schiffeEmpfangen(d, vonId) {
    if (!d || !d.t) return;
    if (d.t === "start") {
      /* RUNDE 92: das Spiel faengt wieder mit dem Verstecken an.
         XANDER: „Man muss sich erst einen Platz suchen, dann beginnt
         die Runde mit einem Countdown." */
      schiffeStand = { phase: d.phase === "verstecken" ? "verstecken" : "schiessen",
                       richter: d.richter || vonId,
                       plaetze: Number(d.plaetze) || SCHIFFE_FELDER,
                       reihe: d.reihe || [], tafel: {}, meins: 0,
                       dran: "", dranName: "", raus: [], countdown: 0,
                       text: d.phase === "verstecken"
                         ? "Such dir heimlich einen Platz — niemand sieht, wohin du gehst."
    ...
    if (d.t === "platz") {
      /* Nur mein eigenes Feld, und nur auf meinem Geraet. Diese
         Zeile steht mit Absicht HINTER der Abfrage oben: kaeme die
         Post vor dem Startruf an, gaebe es noch gar keinen Stand,
         in den sie sich eintragen koennte. */
      schiffeStand.meins = Number(d.nr) || 0;
      schiffeStand.text = "Du liegst auf Platz " + schiffeStand.meins + ".";
      schiffeZeichnen();
      return;
    }
    if (d.t === "aus") {
    ...
    if (d.t === "wartet") {
      /* NUR EINE ZAHL. Namen stuenden hier frueher — und wer weiss,
         ...
         soll. XANDER: „In dem Modus muss man wirklich versteckt
         bleiben koennen." */
      if (typeof d.fertig === "number") {
        schiffeStand.text = d.fertig + " von " + (d.von || d.fertig)
          + " haben sich versteckt.";
      } else {
        schiffeStand.text = d.fehlt ? "Es fehlen noch " + d.fehlt + "." : "Alle sind versteckt.";
      }
      schiffeZeichnen();
      return;
    }
    if (d.t === "belegt") {
      schiffeStand.text = "Platz " + d.nr + " ist schon belegt — such dir einen anderen.";
      schiffeZeichnen();
      return;
    }
    if (d.t === "countdown") {
      schiffeStand.phase = "countdown";
      schiffeStand.text = (d.grund || "Es geht los") + " — gleich geht die Suche los.";
      var sek = Number(d.sek) || 3;
      schiffeStand.countdown = sek;
      schiffeZeichnen();
      var runter = setInterval(function () {
        if (!schiffeStand) { clearInterval(runter); return; }
        schiffeStand.countdown -= 1;
        if (schiffeStand.countdown <= 0) { clearInterval(runter); schiffeStand.countdown = 0; }
        schiffeZeichnen();
      }, 1000);
      return;
    }
    if (d.t === "los") {
      ...
      /* Der Text bleibt stehen. Frueher wurde er hier geleert — das
         war richtig, solange darin nur „Alle sind versteckt" stand.
         Jetzt steht darin, auf welchem Platz man selbst liegt, und
         das ist das Einzige, was man ueber das Spiel weiss. */
      schiffeZeichnen();
      return;
    }
    if (d.t === "schuss") {
      schiffeStand.tafel[d.nr] = d.treffer ? "treffer" : "daneben";
      /* RUNDE 100 — der Ton gehoert zu DIESEM Schuss, nicht zu jedem
         Neuzeichnen des Bretts (siehe DMA_SCHIFFE in app.js). */
      schiffeStand.letzter = { nr: Number(d.nr) || 0, treffer: Boolean(d.treffer), zeit: Date.now() };
      schiffeStand.dran = d.dran || "";
      schiffeStand.dranName = d.dranName || "";
    ...
    /* „belegt" und „wartet" stammen aus der Zeit, in der man sich
       sein Versteck aussuchte. Sie werden nicht mehr gesendet; der
       Empfang bleibt nur stehen, damit ein Geraet mit alter Fassung
       im Raum nichts kaputtmacht. */
  }
  ...
  function schiffeWahl(nr) {
    ...
    /* RUNDE 92: in der ersten Haelfte des Spiels bedeutet ein Tipp
       „da verstecke ich mich" — und geht als persoenliche Post an den
       Schiedsrichter, nicht an den Raum. */
    if (schiffeStand.phase === "verstecken") {
      schiffeStand.meins = nr;
      schiffeStand.text = "Du suchst dir Platz " + nr + " \u2026";
      schiffeZeichnen();
      return schiffeAnRichter({ t: "versteck", nr: nr });
    }
    if (schiffeStand.phase === "countdown") return false;
  ...
  var kontenJe = {};
  /* FUNK 95 — SPIELSYSTEM: jede Person traegt ihre Spiel-Kennung (die
     Supabase-Kennung: Konto oder anonyme Gastsitzung) in jedem Paket mit.
     Damit weiss jedes Geraet, WEN ein Treffer auf dem Server meint. */
  var spielKennung = "";
  var spielJe = {};
  function spielMerken(id, k) {
    if (!id || typeof k !== "string" || !/^[0-9a-f-]{36}$/i.test(k)) return;
    spielJe[id] = k;
  }
  function spielIdVon(id) {
    if (!id) return "";
    if (id === zustand.ichId) return spielKennung;
    var k = spielJe[id] || kontoVon(id) || "";
    return /^[0-9a-f-]{36}$/i.test(k) ? k : "";
  }
  function spielGesperrt() {
    try { return Boolean(window.DMA_SPIEL && window.DMA_SPIEL.gesperrt && window.DMA_SPIEL.gesperrt()); } catch (e) { return false; }
  }
  function spielGewechselt() {
    try { if (window.DMA_SPIEL && window.DMA_SPIEL.platzGewechselt) window.DMA_SPIEL.platzGewechselt(); } catch (e) {}
  }
  ...
  var BEFEHLE = [
    ...
    { gr: "reden", w: "hotrodfahrt", kurz: "hotrodfahrt", nutzt: "/hotrodfahrt 5", was: "im Hot Rod zu Platz 5 fahren \u2014 auch mit gemaltem Weg (/hotrodfahrt 1-5-6)" },
    { gr: "reden", w: "monstertruck", kurz: "monstertruck", nutzt: "/monstertruck 5", was: "im Monstertruck zu Platz 5 donnern \u2014 auch mit gemaltem Weg" },
    { gr: "reden", w: "zuenden", kurz: "zuenden", nutzt: "/zuenden Name", was: "jemanden als Rakete z\u00fcnden: Lunte, Start, Feuerwerk oben, ru\u00dfig zur\u00fcck" },
    { gr: "reden", w: "flug", kurz: "fliegen", nutzt: "/flug 5",
      was: "Flugzeug \u2014 dein Bild sitzt hinter dem Fenster und wird auf Platz 5 abgesetzt; mit gemaltem Weg auch \u00fcber Umwege (/flug 1-5-6)" },
    { gr: "reden", w: "maulwurf", kurz: "graben", nutzt: "/maulwurf 5",
      was: "Maulwurf \u2014 du gr\u00e4bst dich unter dem Raum hindurch und kommst auf Platz 5 wieder heraus; mit gemaltem Weg auch \u00fcber Umwege (/maulwurf 1-5-6)" },
    ...
    { gr: "reden", w: "waschmaschine", kurz: "waschen", nutzt: "/waschmaschine Name",
      was: "sein Bild wird zum Bullauge \u2014 Wasser rein, waschen, schleudern, Tuer auf" },
    ...
    /* RUNDE 88 — XANDER: „Die Animation mit dem Kaninchen … als
       einfacher Profil-Effekt … und dass ich andere damit
       beeinflussen kann. Und die Reise ist ein anderer Effekt."
       Ohne diese Zeile gaebe es zwar die Zeichnung, aber
       befehlBekannt() wuerde „/kaninchen" als Vertipper abweisen und
       die Zeile ginge gar nicht erst hinaus — genau der Fehler, den
       pruefe-effekttueren seit Fassung 360 bewacht. */
    { gr: "reden", w: "kaninchen", kurz: "zauberstab", nutzt: "/kaninchen [Name]",
      was: "Kaninchen aus dem Zylinder \u2014 Hut drueber, dreimal mit dem Zauberstab geklopft, an den Ohren heraus, Beifall" },
    ...
    /* RUNDE 100 — Stadt · Land · Fluss IM Klassenzimmer (Walkie #87). */
    { gr: "schule", w: "slf", kurz: "stadtlandfluss", nutzt: "/slf",
      was: "Stadt \u00b7 Land \u00b7 Fluss im Raum \u2014 alle schreiben gleichzeitig unter der B\u00fchne, \u201eStopp!\u201c beendet die Runde; /slf aus beendet das Spiel" },
...
{ gr: "reden", w: "vogelkot", kurz: "kot", nutzt: "/vogelkot Name", was: "Vogel \u2014 er fliegt dar\u00fcber und trifft von oben" },
    ...
    { gr: "reden", w: "hotrod", kurz: "aufziehauto", nutzt: "/hotrod Name x3",
    ...
    { gr: "feier", w: "mario",     kurz: "pilz",   nutzt: "/mario [Name oder 5-6-7]", was: "Mario-Modus: unter der oberen Reihe entlanghüpfen und an die Plätze darüber schlagen — heraus kommen Münzen oder ein Pilz, und die Punkte zählen mit. Mit gezeichnetem Weg sind Besetzte Gegner" },
    ...
    { gr: "schule", w: "aufgabe", kurz: "frage",     nutzt: "/aufgabe <Text>", was: "Das Aufgaben-Modul: /aufgabe allein öffnet die Tafel mit allen acht Arten. Mit Text dahinter stellst du eine Aufgabe in eigenen Worten — was die anderen danach schreiben, gilt als Antwort und kann benotet werden (läuft schon eine, beendet /aufgabe sie)" },
  ...
  var KURZ = { msg: "w", m: "w", query: "w", fluester: "w", whisper: "w",
                ...
                /* RUNDE 90 — XANDER sagt „Flugzeug", der Befehl heisst
                   seit jeher „/flug": „Alternativ koennte ich mir in
                   der Zukunft noch im Flugzeug vorstellen." Die Reise
                   gibt es laengst — nur das Wort, das er benutzt, fiel
                   bisher ins Leere. Jetzt fuehren beide zum selben. */
                flugzeug: "flug", flieger: "flug", jet: "flug",
                ...
                /* RUNDE 101 — Xander: „Hau ab oder Abstand" und „das
                   wegschicken". */
                wegschieben: "hauab", wegschicken: "hauab", schubsen: "hauab",
                hotroad: "hotrod", aufzieh: "hotrod",
                /* „muenze" gehoert NICHT hierher: den Befehl /muenze gibt
                   es schon (der Geldeffekt), und ein Kurzwort, das einen
                   echten Befehl verdeckt, nimmt ihm die Tuer. */
                pilz: "mario", supermario: "mario", klempner: "mario",
  ...
  /* =========================================================
     RUNDE 100 — AUCH MICH SELBST FINDEN
     ---------------------------------------------------------
     XANDER (Walkie-Talkie, zur Leiter): „Ich finde niemand mit dem
     Namen XanderFox im Raum steht da."
     GEFUNDEN: personNachName sucht in zustand.leute — dort stehen
     nur die ANDEREN. Tippt man im Platzmenue auf das eigene Bild, geht
     aber „/leiter XanderFox" hinaus, mit dem eigenen Namen. Der Befehl
     fand niemanden und brach ab — die Leiter erschien nie. Dasselbe beim
     Kran: „Der Effekt soll ausserdem auch immer mit mir selbst gehen."
     Fuer Befehle, die auch einem selbst gelten duerfen, gibt es deshalb
     diese Suche: zuerst der eigene Name, dann die anderen. */
  function personOderIch(name) {
    var k = String(name || "").trim().toLowerCase();
    if (!k) return null;
    if (zustand.ichName && String(zustand.ichName).toLowerCase() === k) {
      return { id: zustand.ichId, name: zustand.ichName, ich: true };
    }
    return personNachName(name);
  }
  ...
  /* =========================================================
     RUNDE 88 — „ICH KANN MICH IMMER NOCH NICHT SELBER ANZIEHEN"
     ---------------------------------------------------------
     XANDER, woertlich: „dann kann ich immer noch nicht das
     Musikstueck fuer mich selber einstellen. Ich kann mich immer
     noch nicht selber anziehen und ausziehen." Und frueher schon:
     „ich moechte meine eigene Musik hoeren … und ich moechte das
     selber auch ausprobieren koennen, wie ich das bei mir auf den
     Kopf setzen soll."

     NACHGESEHEN, WORAN ES LAG — und es war kein fehlender Effekt:
     jeder Befehl sucht sein Ziel mit personNachName(), und diese
     Suche geht ueber zustand.leute. Darin stehen ALLE AUSSER MIR;
     der eigene Platz kommt aus zustand.ichName und liegt gar nicht
     in der Liste. Wer „ich" schrieb, fand also niemanden, und der
     Rueckfall { name: „ich" } suchte einen Platz, der „ich" heisst.
     Es gibt keinen. Der Effekt ging ins Leere.
     Mit dem eigenen NAMEN ging es uebrigens schon immer — die
     Platzsuche in app.js (lcZielPlaetze) vergleicht die Namen unter
     den Plaetzen, und der eigene steht dort auch. Nur kommt niemand
     auf die Idee, sich selbst mit Namen anzureden.

     Jetzt heisst „ich", „mich", „mir", „selbst", „selber" (und
     englisch „me", „myself", „self") genau das: mein eigener Platz.
     Das gilt fuer JEDEN Befehl, der einen Platz trifft — also auch
     fuer /anziehen, /ausziehen und /kopfhoerer, die er genannt hat. */
  var SELBST_WORT = /^(ich|mich|mir|selbst|selber|mir\s+selbst|mich\s+selbst|me|myself|self)$/i;
  function selbstStattName(roh) {
    var t = String(roh || "").trim();
    return SELBST_WORT.test(t) ? (zustand.ichName || t) : t;
  }
  /* Ein Ziel fuer eine Wirkung am Platz: erst „ich" aufloesen, dann
     wie bisher suchen. Ohne Treffer bleibt der getippte Name stehen —
     die Platzsuche in app.js ist nachsichtiger als diese hier. */
  function zielPerson(roh) {
    var t = selbstStattName(roh);
    return personNachName(t) || praesenzNachName(t) || { name: t };
  }
  ...
  var ZUSATZ_FELDER = [
    ...
    "wie",
    /* RUNDE 101 — wie oft der Hot Rod aufgezogen wurde (1×, 3×, 5×).
       XANDER (Funk 67): „Er fährt immer gleich schnell.“ GEFUNDEN im
       Prüflauf (pruefe-zusatzfelder): das Feld fehlte hier — bei allen
       ANDEREN kam der Hot Rod deshalb immer mit Kraft 1 an. */
    "kraft"
  ];
  ...
  function postEmpfangen(n) {
    ...
    if (n.art === "slfpost") {
      slfLiveBlatt(n.slf || {}, n.von, n.vonName || "");
      return;
    }
    if (n.art === "spielpost") {
    ...
    /* RUNDE 100 — LEHRER-PANEL: „Buehne leeren". Der Lehrer schickt jedem
       auf der Buehne diese Post; wer sie bekommt, geht selbst herunter —
       genau wie beim Pac-Man entscheidet jedes Geraet fuer sich. */
    if (n.art === "runter" && (!n.raum || n.raum === zustand.raum)) {
      try { buehneSetzen(false); } catch (e) {}
      systemZeile("Der Lehrer hat die Bühne frei gemacht — tipp auf einen freien Platz, wenn du wieder hinauf möchtest.");
      return;
    }
    if (n.art === "redeverbot") {
  ...
  function befehlAusfuehren(roh) {
    ...
    /* RUNDE 100 — die Uebungspuppe (siehe puppeUmschalten). */
    if (wort === "puppe" || wort === "dummy") return puppeUmschalten(rest) || true;
    /* FUNK 95 — das Spielfenster: /spiel oeffnet es, /lernen holt gleich
       eine Deutsch-Aufgabe (die einzige Quelle fuer Punkte). */
    if (wort === "spiel" || wort === "kampf") {
      if (window.DMA_SPIEL) window.DMA_SPIEL.menue();
      return true;
    }
    if (wort === "lernen" || wort === "deutsch") {
      if (window.DMA_SPIEL) window.DMA_SPIEL.aufgabe();
      return true;
    }
    /* FUNK 89 — kaputt heisst: nicht reisen, nicht wechseln, bis zum
       Pflaster. Gesperrt sind die Befehle, mit denen man SICH SELBST
       bewegt (ohne Namen, mit einer Platznummer oder dem eigenen Namen). */
    if (spielGesperrt() && /^(tausch|flug|maulwurf|portal|boot|delfin|kran|dampfer|lok|liane|feder|frosch|zylinder|fahrstuhl|beamen|rohr|heli|pferd|greifvogel|turm|untertasse|mieze|frisbee|gotteshand|pranke|mannhand|frauenhand|hexenhand|roboterhand|fahren|hotrod|leiter|gemeinsam|bagger|schaufel)$/.test(wort)) {
      var rNamen = rest.replace(/[x\u00d7]\d\s*$/i, "").replace(/\d+/g, "").trim().toLowerCase();
      if (!rNamen || rNamen === String(zustand.ichName || "").toLowerCase() || rNamen === "ich" || rNamen === "mich") {
        return systemZeile(window.DMA_SPIEL.gesperrtGrund());
      }
    }
    ...
    if (art === "leck") {
      var wen3 = rest ? (zielPerson(rest)) : null;
    ...
    if (art === "box") {
      var wen4 = rest ? (zielPerson(rest)) : null;
    ...
    if (art === "tausch") {
      ...
      if (!rest) {
        ...
        sitzSchicken([zustand.ichId], zustand.ichName + " setzt sich auf Platz " + (frei + 1) + ".");
      ...
      sitzSchicken([zustand.ichId, wenT.id], satz);
      systemZeile(satz);
      melden();
      return;
    }
    ...
    if (art === "lasso") {
      ...
      /* RUNDE 100 — XANDER (Walkie #88): „Die Person landet nicht neben
         mir." GEFUNDEN (Puppe auf Platz 2, ich auf 1): „unter mir" (5)
         zaehlte genauso nah wie „neben mir" (2) — und sass die Person
         SCHON neben mir, zog das Lasso sie von dort weg nach unten.
         Jetzt: eine Reihe weiter zaehlt 1,5 Plaetze, links/rechts in
         derselben Reihe gewinnt also immer; und wer schon direkt neben
         mir sitzt, bleibt sitzen. */
      var lageL = function (n) { return { r: Math.floor((n - 1) / 4), s: (n - 1) % 4 }; };
      var ichL = lageL(meinerL.nummer);
      var naeheL = function (n) {
        var l = lageL(n);
        return Math.abs(l.r - ichL.r) * 1.5 + Math.abs(l.s - ichL.s);
      };
      var seineLage = lageL(seinerL.nummer);
      if (seineLage.r === ichL.r && Math.abs(seineLage.s - ichL.s) === 1) {
        return systemZeile(wenL.name + " sitzt schon direkt neben dir.");
      }
    ...
    /* RUNDE 101 — XANDER (Funk #39): „einen Schaufelradbagger [der
    ...
    if (art === "bagger" || art === "schaufelradbagger") {
      if (zustand.lage !== "drin") return systemZeile("Dafuer musst du erst im Raum sein.");
      var restB = rest.trim();
      if (!restB) return systemZeile("So geht es:  /bagger Nickname 3 \u2014 "
        + "der Schaufelradbagger holt ihn und l\u00e4dt ihn auf Platz 3 ab.");
      baggerZieht = true;
      try { return befehlAusfuehren("/heb " + restB); }
      finally { baggerZieht = false; }
    }

    /* =================================================================
       RUNDE 101 — HAU AB / ABSTAND
       -----------------------------------------------------------------
       XANDER (Funk #18): „… jemand mit seinen Händen realistisch von
       sich weg schieben kann, falls einem jemand zu nah kommt … in jede
       Richtung … Der andere wandert dabei einen Platz weiter und bleibt
       dann dort. Du kannst die Animation Hau ab oder Abstand nennen."
       „Einen Platz weiter" heisst: der naechste Platz in SCHUBRICHTUNG
       (von mir zu ihm), gemessen an der Sitzreihe, wie sie auf diesem
       Geraet steht. Die Nummer reist mit, also sieht jedes Geraet
       dasselbe Ziel. Umgesetzt wird an derselben Stelle wie bei /heb. */
    /* RUNDE 101 — XANDER (Funk #40): „dass man jemand anderen aufziehen
       kann und ihn als Auto losflitzen lassen kann … Profilbild auf zwei
       Mini-Rädern mit den seitlich verchromten Auspüffen eines Hot Rods,
       mit ganz viel Dampf, der die anderen Gesichter … bleibend
       schmutzig macht." Die Zahl der Umdrehungen („x3") faehrt mit, wie
       beim eigenen Fahren; sie macht ihn schneller. Kein Platzwechsel. */
    if (art === "hotrod") {
      if (zustand.lage !== "drin") return systemZeile("Dafuer musst du erst im Raum sein.");
      var teileR = rest.trim().split(/\s+/).filter(Boolean);
      var kraftR = 1;
      if (teileR.length && /^x?[1-5]$/i.test(teileR[teileR.length - 1])) {
        kraftR = parseInt(teileR.pop().replace(/x/i, ""), 10);
      }
      var nameR = teileR.join(" ");
      if (!nameR) return systemZeile("So geht es:  /hotrod Nickname x3 \u2014 "
        + "dreimal aufgezogen, dann flitzt er als Hot Rod an allen vorbei.");
      var wenR = personOderIch(nameR);
      if (!wenR) return systemZeile("Ich finde niemanden mit dem Namen \u201e" + nameR + "\u201c im Raum.");
      var wenNameR = wenR.ich ? "sich selbst" : wenR.name;
      return anAlle("aktion", zustand.ichName + " zieht " + wenNameR
        + (kraftR > 1 ? " " + kraftR + "\u00d7" : "") + " auf und l\u00e4sst "
        + (wenR.ich ? "sich" : wenR.name) + " als Hot Rod losflitzen  \ud83c\udfce\ufe0f",
        { wirkung: "hotrod", wen: wenR.name, kraft: String(kraftR) });
    }

    if (art === "hauab") {
      if (zustand.lage !== "drin") return systemZeile("Dafuer musst du erst im Raum sein.");
      var nameA = rest.trim();
      if (!nameA) return systemZeile("So geht es:  /hauab Nickname \u2014 "
        + "du schiebst ihn mit beiden H\u00e4nden einen Platz weiter weg.");
      var wenA = personOderIch(nameA);
      if (!wenA) return systemZeile("Ich finde niemanden mit dem Namen \u201e" + nameA + "\u201c im Raum.");
      if (wenA.ich) return systemZeile("Dich selbst kannst du nicht wegschieben.");
      var plA = plaetzeBauen(), meinerA = null, seinerA = null;
      plA.forEach(function (pl) { if (pl.ich) meinerA = pl; if (pl.id === wenA.id) seinerA = pl; });
      if (!meinerA) return systemZeile("Du sitzt auf keinem Platz \u2014 von wo aus willst du schieben?");
      if (!seinerA) return systemZeile(wenA.name + " sitzt gerade auf keinem Platz.");
      var lagenA = {};
      try {
        document.querySelectorAll("#lcPlaetze .lc-platz[data-lc-platz]").forEach(function (el) {
          var k = el.querySelector(".lc-kreis") || el, r = k.getBoundingClientRect();
          if (r.width > 0) lagenA[Number(el.getAttribute("data-lc-platz"))] =
            { x: r.left + r.width / 2, y: r.top + r.height / 2, w: r.width };
        });
      } catch (e) {}
      var MA = lagenA[meinerA.nummer], TA = lagenA[seinerA.nummer];
      if (!MA || !TA) return systemZeile("Die Sitzreihe ist gerade nicht zu sehen \u2014 "
        + "\u00f6ffne das Klassenzimmer und versuch es noch einmal.");
      var dA = Math.hypot(TA.x - MA.x, TA.y - MA.y) || 1;
      /* „falls einem jemand zu nah kommt": nur die direkten Nachbarn,
         auch schraeg. */
      if (dA > TA.w * 1.9) return systemZeile(wenA.name + " sitzt nicht direkt neben dir \u2014 "
        + "wegschieben geht nur, wenn dir jemand zu nah kommt.");
      var uxA = (TA.x - MA.x) / dA, uyA = (TA.y - MA.y) / dA, bestA = 0, kostA = Infinity;
      Object.keys(lagenA).forEach(function (nr) {
        nr = Number(nr);
        if (nr === meinerA.nummer || nr === seinerA.nummer) return;
        var c = lagenA[nr], vx = c.x - TA.x, vy = c.y - TA.y, lang = Math.hypot(vx, vy) || 1;
        var vor = vx * uxA + vy * uyA;
        if (vor < TA.w * 0.3) return;                      /* nicht zurueck, nicht quer */
        var kost = lang * (1 + 2 * (1 - vor / lang));      /* nah UND in Schubrichtung */
        if (kost < kostA) { kostA = kost; bestA = nr; }
      });
      if (!bestA) {
        /* Am Rand: dann der naechste Platz, der weiter von mir weg ist. */
        Object.keys(lagenA).forEach(function (nr) {
          nr = Number(nr);
          if (nr === meinerA.nummer || nr === seinerA.nummer) return;
          var c = lagenA[nr], abMir = Math.hypot(c.x - MA.x, c.y - MA.y);
          if (abMir <= dA + TA.w * 0.2) return;
          var kost = Math.hypot(c.x - TA.x, c.y - TA.y);
          if (kost < kostA) { kostA = kost; bestA = nr; }
        });
      }
      if (!bestA) return systemZeile("Hinter " + wenA.name + " ist kein Platz mehr frei, "
        + "auf den du schieben k\u00f6nntest.");
      hauabZieht = true;
      try { return befehlAusfuehren("/heb " + wenA.name + " " + bestA); }
      finally { hauabZieht = false; }
    }
    ...
    if (art === "heb") {
      ...
      var wenH = personOderIch(nameH);   /* Runde 100: auch sich selbst heben */
      ...
      var satzH = zustand.ichName + (wieH === "heber"
        ? " hebt " + wenNameH + " auf Platz " + nummerH
        : wieH === "kranheben"
      ...
      var meinPlatzH = meinerH ? meinerH.nummer : 0;
      var schickenH = function () {
        try {
          if (zustand.raum !== raumH) return;
          /* RUNDE 99 — hat sich in den 2,3 s etwas verschoben (jemand
             anderes hat ihn oder mich schon woanders hingezogen), dann
             wird NICHT mehr umgesetzt: sonst verknotet es sich. */
          var jetztH = plaetzeBauen(), nochDa = false, ichNoch = !meinPlatzH, zielFrei = true;
          jetztH.forEach(function (pl) {
            if (pl.id === wenH.id && pl.nummer === seinerH.nummer) nochDa = true;
            if (pl.ich && pl.nummer === meinPlatzH) ichNoch = true;
            if (pl.nummer === nummerH && !pl.leer && (!dortH || pl.id !== dortH.id)) zielFrei = false;
          });
          if (!nochDa || !ichNoch || !zielFrei) return;
          /* RUNDE 98 — HIER, und keinen Wimpernschlag frueher. */
          sitzTausch[wenH.id] = nummerH - 1;
          if (dortH) sitzTausch[dortH.id] = seinerH.nummer - 1;
          sitzSchicken([wenH.id, dortH && dortH.id], satzH);
          melden();
        } catch (e) {}
      };
      /* Der Kran setzt spaeter ab als die Angel: sein Seil geht erst
         wieder herunter (lcGekrantR98, 88 % von 2,6 s = 2288 ms). */
      /* RUNDE 99: die Angel wirft jetzt erst aus, beisst, holt ein —
         abgesetzt ist bei 3150 ms (lcHeber in app.js, T_AB). */
      /* RUNDE 100 — das Lasso zieht jetzt bis ganz auf den Zielplatz, in
         drei Zuegen: der dritte sitzt bei 86 % von 3,4 s = 2924 ms. Erst
         danach wird umgesetzt (vorher 2300 ms — mitten im zweiten Zug,
         XANDER: „Person ist zu frueh da"). */
      /* RUNDE 101 — der Bagger laedt bei 80 % von 3,6 s ab und federt
      ...
      offeneZuege.push({ werfer: zustand.ichName, wen: wenH.name, ziel: nummerH,
                         bis: Date.now() + dauerH + 200, uhr: setTimeout(schickenH, dauerH) });
    ...
    if (art === "herz") {
      var wem = rest ? (zielPerson(rest)) : null;
    ...
    if (art === "konfetti") {
      var anWen = rest ? (zielPerson(rest)) : null;
    ...
    if (art === "ballon") {
      var fuerWen = rest ? (zielPerson(rest)) : null;
    ...
    if (art === "geschenk") {
      var wemGe = rest ? (zielPerson(rest)) : null;
    ...
    if (GROSSGESCHENK[art]) {
      ...
      var wemG = rest ? (zielPerson(rest)) : null;
    ...
    if ((art === "flug" || art === "maulwurf" || art === "portal"
      ...
      /* =========================================================
         ...
         ---------------------------------------------------------
         XANDER: „das Weg zeichnen … das soll auch fuer das
         Flugzeug, die Sprungfeder und den Maulwurf gehen."
         Bisher nahm dieser Zweig nur EINE Zahl an; malte man einen
         Weg und tippte dann „Flugzeug", fiel die Kette hier durch
         und landete als Nickname im Chat. Jetzt darf hinter diesen
         drei Befehlen dieselbe Kette stehen wie beim Fahren
         („/flug 1-5-6"): die letzte Zahl ist das Ziel, die davor
         sind die Stationen. Sie faehrt unveraendert mit, damit
         JEDES Geraet denselben Umweg zeichnet.
      ...
      var ueberR = (kettenR && stationenR.length > 2)
        ? " \u00fcber " + stationenR.slice(1, -1).join(", ") : "";
      var wenR = (kettenR && stationenR.length > 1)
        ? stationenR.join("-") : String(zielNrR);
      /* =========================================================
    ...
    if ((art === "fahren" || art === "huepfen" || art === "laufen" || art === "hotrodfahrt" || art === "monstertruck")
      ...
      if (art === "hotrodfahrt" || art === "monstertruck") {
        return anAlle("aktion", zustand.ichName
          + (art === "hotrodfahrt" ? " f\u00e4hrt im Hot Rod zu Platz " : " donnert im Monstertruck zu Platz ")
          + platzNr + ueber + "  " + (art === "hotrodfahrt" ? "\ud83c\udfce\ufe0f" : "\ud83d\udefb"),
          { wirkung: art, wen: kette, tempo: String(tempoF) });
      }
      return anAlle("aktion", zustand.ichName
    ...
    /* =========================================================
       ...
       ---------------------------------------------------------
       XANDER: „wenn ich mich unter der oberen Reihe bewege, dass
       ich wie bei Super Mario an die Plaetze dran schlagen kann,
       wo Leute sitzen, und dann Power-up rauskommt oder Muenzen
       … und man kann auch den Weg einzeichnen, auch ueber die
       Benutzer zeichnen, und wenn man das macht, dann sind die
       Benutzer halt Gegner."

       Dieser Zweig steht VOR AM_PLATZ: dort ist ein Name Pflicht,
       und „/mario" OHNE Namen ist hier gerade der Normalfall — er
       laeuft dann seine eigene Reihe entlang. Drei Formen:
         /mario          die eigene Reihe entlang, Bloecke schlagen
         /mario 5-6-7    der gemalte Weg; Besetzte sind Gegner
         /mario Bea      bis zu Bea
       Das „los" faehrt mit, damit auf JEDEM Geraet dieselben
       Muenzen aus denselben Bloecken kommen. */
    if (art === "mario") {
      var ketteM = String(rest || "").replace(/\s+/g, "");
      var istKetteM = /^\d+(-\d+)+$/.test(ketteM);
      var zielM = istKetteM ? ketteM : String(rest || "").trim();
      var wortM = istKetteM
        ? " springt als Mario bis Platz " + ketteM.split("-").pop()
        : (zielM ? " springt als Mario zu " + zielM
                 : " springt als Mario unter der oberen Reihe entlang");
      return anAlle("aktion", zustand.ichName + wortM + "  \ud83c\udf44",
        { wirkung: "mariolauf", wen: zielM, los: Math.random().toFixed(4) });
    }
    ...
    /* ---- UND EINS AUF DIE OHREN ----
       ...
       =========================================================
       RUNDE 88 — DIESE ABZWEIGUNG STAND AN DER FALSCHEN STELLE
       ---------------------------------------------------------
       Der Satz „Diese Abzweigung muss VOR AM_PLATZ stehen" stand
       hier seit Runde 80 — sie stand aber DAHINTER, rund 470 Zeilen
       tiefer. Und AM_PLATZ kennt „kopfhoerer" auch. Also hat AM_PLATZ
       jeden Aufruf vorher abgefangen, und hier kam nie einer an.
       GEMESSEN an „/kopfhoerer Bea 1" (mit sechs Liedern im Ordner):
       es entstand die Zeile „Alex setzt Kopfhörer auf Bea 1" mit
       wen: „Bea 1" — ein Platz mit diesem Namen existiert nicht, es
       lief also weder ein Lied noch traf der Effekt jemanden.
       GENAU DAS hat er gemeldet: „dann kann ich immer noch nicht das
       Musikstück für mich selber einstellen" und „ich möchte die
       Einstellung dafür bei mir haben, was ich den Leuten einstelle,
       was ich ihnen auf den Kopfhörer setze."
       Jetzt steht sie wirklich vor AM_PLATZ. Und sie bricht nicht
    ...
    if (art === "leiter") {
      ...
      var wenL = nameL ? personOderIch(nameL) : null;   /* Runde 100: siehe personOderIch */
      if (nameL && !wenL) {
      ...
      setTimeout(function () {
        try {
          ...
          sitzSchicken([idL, dortL && dortL.id], satzL);
          melden();
        } catch (e) {}
    ...
    if (art === "kopfhoerer" && rest && /\s/.test(rest.trim())) {
      ...
      var wemK = zielPerson(namK);
      /* Kein Lied dahinter? Dann war es kein Liedwunsch, sondern ein
         Name mit Leerzeichen. Nicht abbrechen — durchfallen lassen,
         AM_PLATZ setzt die blossen Kopfhoerer auf. */
      if (liedK) {
      var zusatzK = stueckK
    ...
    if (art === "gemeinsam") {
      ...
      /* RUNDE 100 — mit sich selbst kann man nicht zu zweit fahren. Vorher
         ging „Alex faehrt gemeinsam los mit Alex" an alle hinaus, und
         nichts bewegte sich (XANDER: „es geht ueberhaupt nicht und
         scheint einen doppelten Eintrag zu haben"). */
      if ((wemG.id && wemG.id === zustand.ichId)
          || String(wemG.name || "").toLowerCase() === String(zustand.ichName || "").toLowerCase()) {
        return systemZeile("Zu zweit geht nur mit jemand anderem: tipp auf sein Bild \u2192 Reisen \u2192 Zu zweit, "
          + "oder /gemeinsam Nickname.");
      }
      return anAlle("aktion", zustand.ichName
        + (stueckG === "ball"
    ...
    /* =========================================================
       ...
       ---------------------------------------------------------
       XANDER: „Pac-Man: eigener Weg zeichnen … ueber die Plaetze."
       Steht statt eines Namens eine Kette da („/pacman 1-2-3-7"),
       ist das der gemalte Weg. Er faehrt unveraendert mit, damit
       JEDES Geraet dieselbe Strecke frisst; im Chat steht das ZIEL
       und nicht die Zahlenreihe — sonst liest sich die Zeile wie
       eine Telefonnummer. */
    if (art === "pacman" && /^\s*\d+(\s*-\s*\d+)+\s*$/.test(rest)) {
      var kettePac = String(rest).replace(/\s+/g, "");
      var zielPac = kettePac.split("-").pop();
      return anAlle("aktion", zustand.ichName
        + " frisst sich als Pac-Man bis Platz " + zielPac + "  \ud83d\udc7e",
        { wirkung: "pacjagd", wen: kettePac });
    }
    ...
    if (art === "slf" || art === "stadtlandfluss") return slfLiveStarten(rest);
    ...
    if (art === "musik") {
      ...
      /* =========================================================
         RUNDE 88 — DER ABSCHNITT GILT AUCH FUER ALLE
         ---------------------------------------------------------
         XANDER: „die Songabschnitte, die ich festlege … sollen fuer
         alle hoerbar werden."
         Steht hinter dem Lied eine Zeit, ist sie der Ausschnitt —
         genau wie beim Kopfhoerer:
           /musik 3 1:18        ab 1:18 bis zum Schluss
           /musik 3 1:18-1:53   genau diese Stelle
         Die Zeit wird VORHER abgeschnitten, sonst suchte
         liedFinden ein Lied namens „3 1:18". */
      var stueckM = null;
      var teileM = wahlM.split(/\s+/);
      if (teileM.length > 1) {
        var letzteM = teileM[teileM.length - 1];
        var mM = /^(\d+(?::[0-5]\d)?)(?:\s*-\s*(\d+(?::[0-5]\d)?))?$/.exec(letzteM);
        if (mM) {
          var sekM = function (t) {
            var teil = String(t).split(":");
            return teil.length === 2 ? Number(teil[0]) * 60 + Number(teil[1]) : Number(teil[0]);
          };
          var abM = sekM(mM[1]);
          var bisM = mM[2] ? sekM(mM[2]) : 0;
          if (!bisM || bisM > abM) {
            stueckM = { ab: abM, bis: bisM };
            teileM.pop();
            wahlM = teileM.join(" ");
          }
        }
      ...
      var zeitwortM = stueckM
        ? " (ab " + Math.floor(stueckM.ab / 60) + ":"
          + String(stueckM.ab % 60).padStart(2, "0")
          + (stueckM.bis
              ? " bis " + Math.floor(stueckM.bis / 60) + ":"
                + String(stueckM.bis % 60).padStart(2, "0")
              : "") + ")"
        : "";
      return anAlle("aktion", zustand.ichName + " legt \u201e" + liedM.titel + "\u201c"
                    + zeitwortM + " auf  \ud83c\udfb5",
                    { wirkung: "musik", lied: liedM.datei, liedTitel: liedM.titel,
                      liedAb: stueckM ? stueckM.ab : 0,
                      liedBis: stueckM ? stueckM.bis : 0 });
    }
    /* ---- DIE SPRUEHDOSE ----
    ...
    if (art === "anziehen" || art === "ausziehen") {
      ...
      /* =========================================================
         RUNDE 88 — „/ausziehen Name" HAT DEN NAMEN GEFRESSEN
         ---------------------------------------------------------
         XANDER: „Ich kann mich immer noch nicht selber anziehen und
         ausziehen."
         HIER WAR DER ZWEITE GRUND (der erste war, dass „ich" keinen
         Platz fand): bei /ausziehen wurde stumpf ein „aus" ANGEHAENGT
         und danach das LETZTE Wort als Kleidungsstueck abgezogen —
         also genau dieses angehaengte „aus". Was uebrig blieb, war
         alles andere, und das wurde zum NAMEN.
         GEMESSEN an „/ausziehen ich krone": Teile [ich, krone, aus],
         abgezogen wird „aus", Name = „ich krone". Es gibt keinen
         Platz, der „ich krone" heisst — der Effekt ging ins Leere,
         und in der Zeile stand „Alex zieht ich krone alles wieder
         aus".
         Jetzt wird bei /ausziehen NICHTS angehaengt: steht am Ende
         ein bekanntes Kleidungsstueck, wird genau das abgenommen,
         sonst alles. Der Name ist, was davor steht — und ohne Namen
         ist man selbst gemeint (wie bei /me). */
      if (!teileA.length) {
      ...
      if (art === "ausziehen") {
        /* Ausziehen ist immer ein „aus". Steht am Ende ein bekanntes
           ...
           zum Namen; steht dort keines, ist der ganze Rest der Name
  ...
  function lautstaerkeVerfolgen(strom) {
    ...
    try {
      ...
      hoerUhr = setInterval(function () {
        ...
        /* WALKIE #248 — Blüte 2: „mach die Sensibilität dann ein bisschen
           schneller so dass man das auch sieht wie das zusammen geht".
           Bei diesem Sprechbild gilt eine Pause schon nach 0,3 s als
           Pause (sonst 0,55 s) — dann sieht man die Blätter auch zwischen
           zwei Sätzen zugehen. */
        var nachlauf = zustand.sprechbild === "bluete2" ? 300 : STILL_MS;
        var neu = redet || (stillSeit && Date.now() - stillSeit < nachlauf);
        neu = Boolean(neu);
  ...
  function archivLaden(raum) {
    ...
    /* Seit Fassung 642 lehnt serverLaden() ab, wenn die Tabelle nicht
       antwortet — zum Nachlesen reicht dann, was im Geraet liegt. */
    return serverLaden(r).then(null, function () { return []; }).then(function (dort) {
  ...
  return {
    ...
    /* RUNDE 100 — der Schiedsrichter zum Nachpruefen: auf Feld nr liegt
       jemand, A schiesst dorthin. Zurueck kommt, was der Schiedsrichter
       an alle geschickt HAETTE — gesendet wird nichts, und das laufende
       Spiel bleibt unberuehrt. XANDER: „Die unteren Reihen sind
       ueberhaupt nicht anwaehlbar." */
    pruefSchiffeSchuss: function (nr) {
      var altSpiel = schiffeSpiel, altAn = schiffeAnAlle, raus = [];
      schiffeAnAlle = function (d) { raus.push(d); };
      schiffeSpiel = { phase: "schiessen", dran: 0, raus: {}, treffer: {},
        reihe: [{ id: "a", name: "A" }, { id: "b", name: "B" }, { id: "c", name: "C" }],
        verstecke: { b: Number(nr) || 0, c: 99 } };
      try { schiffeSchuss(Number(nr) || 0, "a", "A"); }
      finally { schiffeAnAlle = altAn; schiffeSpiel = altSpiel; }
      return raus;
    },
    ...
    effektBefehle: function () {
      ...
      /* RUNDE 88 — und „mariolauf" haengt an „/mario". Der Befehl hat
         eine eigene Abzweigung, weil er auch OHNE Namen gilt — in
         AM_PLATZ waere ein Name Pflicht, und „/mario" allein ist
         gerade der Normalfall („wenn ich mich unter der oberen Reihe
         bewege"). Die Tuer ist also da, sie steht nur nicht in einer
         Tabelle. */
      w.push("mariolauf");
    ...
    /* FASSUNG 679 — das Spiel meldet: wer mitspielt, hat sich geändert → Sitzordnung neu. */
    spielSitzeNeu: function () { melden(); },
    spielSitzTabelle: function () { return Object.assign({}, spielSitz); },
    ...
    /* RUNDE 100 — fuer das Lehrer-Panel (app.js, lcLehrerPanel):
       XANDER: „Beim Schiffe versenken und den anderen Aufgaben moechte
       ich ein kleines Lehrer-Panel beim Aufrufen meines eigenen Profils
       finden, was nur ich als Betreiber habe." */
    aufgabeLaeuft: function () { return Boolean(offeneAufgabe); },
    /* RUNDE 100 — Lehrer-Panel, zweiter Schritt: „Wer ist dran –
       weitergeben / ueberspringen", „Alle stumm / Ton an",
       „Buehne leeren". */
    dranStand: function () {
      var dranJetzt = offeneAufgabe ? String(offeneAufgabe.dran || "") : "";
      var namen = rundeNamen().map(function (p) { return p.name; })
        .filter(function (nm) { return nm && nm !== "Puppe"; });
      return { laeuft: Boolean(offeneAufgabe && dranJetzt), dran: dranJetzt,
               naechster: dranJetzt ? naechsterDran(dranJetzt) : "", namen: namen };
    },
    dranGeben: function (name) {
      if (!offeneAufgabe || !name) return false;
      var ok = dranSetzen(name);
      if (ok) anAlle("system", "\u25b6\ufe0f Jetzt ist " + name + " dran.");
      return ok;
    },
    alleStummStand: function () { return Boolean(zustand.alleStumm); },
    alleStumm: function (an) {
      if (!zustand.haeuptling) return systemZeile("Stummschalten darf nur der Häuptling.");
      an = an !== false;
      zustand.alleStumm = an;
      Object.keys(zustand.leute).forEach(function (id) {
        if (istPuppe(id)) return;
        zustand.stumm[id] = an;
        postSenden(id, { art: "redeverbot", an_: an, raum: zustand.raum });
      });
      return anAlle("system", an ? "\ud83d\udd07 Alle sind stummgeschaltet — schreiben geht weiter, sprechen nicht."
                                 : "\ud83d\udd0a Alle dürfen wieder sprechen.");
    },
    buehneLeeren: function () {
      if (!zustand.haeuptling) return systemZeile("Die Bühne frei machen darf nur der Häuptling.");
      var zahl = 0;
      plaetzeBauen().forEach(function (pl) {
        if (pl.leer || pl.ich || istPuppe(pl.id)) return;
        postSenden(pl.id, { art: "runter", raum: zustand.raum });
        zahl++;
      });
      if (!zahl) return systemZeile("Auf der Bühne sitzt außer dir niemand.");
      return anAlle("system", "\ud83e\uddf9 Die Bühne ist frei — wer hinauf möchte, tippt auf einen freien Platz.");
    },
    aufgabeBeenden: function () { return offeneAufgabe ? aufgabeFreiStellen("") : false; },
    schiffeLaeuft: function () { return Boolean(schiffeStand); },
    /* Runde 100 — Stadt · Land · Fluss im Raum. */
    slfLaeuft: function () { return Boolean(slfLive && slfLive.phase !== "ergebnis"); },
    slfStand: function () { return slfLive; },
    slfEintrag: function (id, wort) {
      if (!slfLive || slfLive.phase !== "schreiben") return false;
      slfLive.blatt[String(id)] = String(wort || "").slice(0, 40);
      return true;
    },
    slfStopp: function () { return slfLiveStopp(); },
    slfSchliessen: function () {
      if (slfLive && slfLive.phase === "ergebnis") { slfLive = null; slfLiveZeichnen(); }
    },
    puppe: function () { return puppeUmschalten(); },
    puppeDa: function () { return puppeDa(); },
    puppeZahl: function () { return puppeZahl(); },
    ...
    /* RUNDE 99 — zu zweit reisen: der, der losfaehrt, setzt BEIDE in
       EINER Nachricht um (siehe sitzSchicken). */
    zuZweitSetzen: function (nrIch, mitName, nrMit) {
      if (zustand.lage !== "drin") return { ok: false };
      var mit = personNachName(mitName);
      if (!mit) return { ok: false };
      var a = Number(nrIch), b = Number(nrMit);
      if (!(a >= 1 && a <= PLAETZE && b >= 1 && b <= PLAETZE) || a === b) return { ok: false };
      var jetzt = plaetzeBauen(), frei = true;
      jetzt.forEach(function (pl) {
        if ((pl.nummer === a || pl.nummer === b) && !pl.leer
            && pl.id !== zustand.ichId && pl.id !== mit.id) frei = false;
      });
      if (!frei) return { ok: false };
      sitzTausch[zustand.ichId] = a - 1;
      sitzTausch[mit.id] = b - 1;
      var satz = zustand.ichName + " und " + mit.name + " sind zu zweit auf Platz "
        + a + " und " + b + " angekommen.";
      sitzSchicken([zustand.ichId, mit.id], satz);
      melden();
      return { ok: true, text: satz };
    },
    ...
    auftritt: function () { return eigenerAuftritt(); },
    auftrittSetzen: function (x) {
      try { if (AUFTRITTE[x]) localStorage.setItem("dma_auftritt", x); else localStorage.removeItem("dma_auftritt"); } catch (e) {}
      return true;
    },
    auftrittZeigen: auftrittZeigen,
    ...
    pruefVerlaufStand: function () {
      return { vomServer: verlaufVomServer, versuche: verlaufVersuche,
               ...
               fehlAnlaeufe: verlaufFehlAnlaeufe, uhrenOffen: verlaufUhrenOffen };
    },
    /* Nur zum Nachmessen (Fassung 642): die Takte des Nachholens
       verkuerzen, damit die Sonde nicht zwanzig Sekunden warten muss.
       Setzt auch „schon geklagt" zurueck. */
    pruefVerlaufTakt: function (pausen, nachfassen, spaeter) {
      if (Array.isArray(pausen)) VERLAUF_PAUSEN = pausen.slice();
      if (Array.isArray(nachfassen)) VERLAUF_NACHFASSEN = nachfassen.slice();
      if (Array.isArray(spaeter)) VERLAUF_SPAETER = spaeter.slice();
      verlaufGeklagt = false;
      return { pausen: VERLAUF_PAUSEN, nachfassen: VERLAUF_NACHFASSEN, spaeter: VERLAUF_SPAETER };
    },
    pruefVerlaufNachreichen: function (anId) { verlaufSchonGeschickt = {}; verlaufNachreichen(anId); },
