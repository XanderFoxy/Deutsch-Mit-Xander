...
(async () => {
  ...
  /* RUNDE 100 — das Bild kippt jetzt bis 15 Grad um den MUND (Xander:
     „mein Profilbild kann dabei leicht kippen"). Der Mund bleibt ueber
     Bea, die Mitte des Umrechtecks wandert dabei um bis zu
     0,22 · Hoehe · sin 15° ≈ 6 px zur Seite — deshalb 8 statt 4 px. */
  sage(Math.abs(schweb.dx) <= 8 && schweb.kinn >= -4 && schweb.kinn <= 20,
  ...
  /* RUNDE 101 — XANDER (Funk 70): die Rueckhand an der Wange ist ein
     EIGENER Effekt (/wange); /streicheln hat seine alte offene Hand
     zurueck. Gemessen wird deshalb hier /wange. */
  console.log("\n3  DIE HAND STREICHELT AN DER WANGE, NICHT UEBERS GESICHT (/wange)\n");
  const str = await abtasten("wange", 3400, `function () {
    const h = document.querySelector(".lc-wange-hand");
    const k = document.querySelector('#lcPlaetze .lc-platz[data-lc-platz="2"] .lc-kreis');
    if (!h || !k) return null;
  ...
  /* RUNDE 100 — XANDER (Walkie #69): „mein Profilbild kann dabei leicht
     kippen, um es besser und lustiger darzustellen" — und am Mund quillt
     der Speichel sichtbar auf. Vorher: hoechstens 3 Grad. */
  {
    const pgS = await br.newPage({ viewport: { width: 390, height: 844 } });
    await pgS.goto("http://127.0.0.1:" + srv.address().port + "/index.html", { waitUntil: "domcontentloaded" });
    await pgS.waitForFunction(() => window.DMA_PRUEF && window.DMA_PRUEFUNG, { timeout: 25000 });
    const kip = await pgS.evaluate(async () => {
      window.DMA_PRUEF.effektBuehne(); await new Promise((f) => setTimeout(f, 300));
      const ich = document.querySelector(".lc-platz-ich .lc-kreis");
      window.DMA_PRUEFUNG.wirkung("sabbern", "Emmi", "Alex", {});
      const t0 = performance.now(); let max = 0, mund = 0;
      while (performance.now() - t0 < 2900) {
        const m = new DOMMatrix(getComputedStyle(ich).transform);
        max = Math.max(max, Math.abs(Math.atan2(m.b, m.a) * 180 / Math.PI));
        const mu = document.querySelector(".lc-sabber-mund");
        if (mu) mund = Math.max(mund, Number(getComputedStyle(mu).opacity));
        await new Promise((f) => requestAnimationFrame(f));
      }
      return { max: Math.round(max), mund };
    });
    await pgS.close();
    sage(kip.max >= 10 && kip.max <= 20, "Sabbern: das eigene Bild kippt sichtbar (10–20 Grad)", kip.max + " Grad");
    sage(kip.mund > 0.9, "Sabbern: am Mund quillt der Speichel auf", "Deckkraft " + kip.mund.toFixed(2));
  }
