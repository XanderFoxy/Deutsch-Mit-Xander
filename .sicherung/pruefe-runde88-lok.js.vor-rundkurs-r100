...
(async () => {
  ...
  const gleis = await pg.evaluate(() => {
    ...
    if (!svg) return null;
    const schienen = Array.from(svg.querySelectorAll(".lc-lok-schiene"))
      .map((p) => p.getAttribute("d"));
    const schwellen = Array.from(svg.querySelectorAll(".lc-lok-schwelle")).map((l) => {
    ...
    return {
      ...
      naheAmPlatz: Array.from(document.querySelectorAll(".lc-platz")).map((pl) => {
        ...
        let best = Infinity;
        svg.querySelectorAll(".lc-lok-schwelle").forEach((l) => {
