...
(async () => {
  ...
  const zaehl = () => pg.evaluate(() => ({ taschen: document.querySelectorAll(".sp-schnell .sp-s-slot").length, stab: Boolean(document.querySelector(".sp-schnell .sp-s-zauber")),
    plus: Boolean(document.querySelector('.sp-schnell [data-s="slotplus"]')), minus: Boolean(document.querySelector('.sp-schnell [data-s="slotminus"]')) }));
  const t0 = await zaehl();
  sage(t0.taschen === 2 && !t0.stab && t0.plus && t0.minus, "Leiste: 2 Taschen, + und −, kein Zauberstab mehr", JSON.stringify(t0));
  await tippe('.sp-schnell [data-s="slotplus"]'); await tick(250);
  await tippe('.sp-schnell [data-s="slotplus"]'); await tick(250);
  const t1 = await zaehl();
  const waffen1 = await pg.evaluate(() => [...document.querySelectorAll(".sp-schnell .sp-s-slot")].map((b) => b.dataset.w));
  sage(t1.taschen === 4 && new Set(waffen1).size === 4, "zweimal +: vier Taschen, jede mit einer anderen Waffe", JSON.stringify(waffen1));
  await tippe('.sp-schnell [data-s="slotminus"]'); await tick(250);
  const t2 = await zaehl();
  const gemerkt = await pg.evaluate(() => localStorage.getItem("dma_spiel_slotzahl"));
  sage(t2.taschen === 3 && gemerkt === "3", "− nimmt eine wieder weg; die Zahl bleibt gemerkt", JSON.stringify({ t2, gemerkt }));
